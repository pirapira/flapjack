...
remove only that module's local generated OLean (and, when the OLean is absent,
its matching generated trace/hash metadata), then rebuild the required target
from source. Do not use a broad `lake clean` for this routine repair.
