...
theorem evalCrepFullProgState_assignList_inv
    [BEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α] [ShiftLeft α] [ShiftRight α]
    [LT α] [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    (functions : List (CompiledFunction α))
  ...
  | nil =>
      ...
      | nil =>
          ...
          | nil =>
              cases targetFuel with
              | zero => simp [crepNestedSeq, evalCrepFullProgState] at hresult
              | succ targetFuel =>
                  simpa [crepNestedSeq, evalCrepFullProgState,
                    updateCrepLocalList] using hresult.symm
          | cons value values => simp [evalCrepFullExpsState] at heval
              ...
              | some headValue =>
                  ...
                  | some tailValues =>
                      ...
                      | succ targetFuel =>
                          ...
                          | zero => simp [crepNestedSeq, evalCrepFullProgState] at hresult
                          | succ targetFuel =>
                              cases htailResult : evalCrepFullProgState
                                  functions primitive ffi sharedMem
                                  baseAddress topAddress (targetFuel + 1)
                                  { state with
                                    locals := updateCrepLocal state.locals slot value }
