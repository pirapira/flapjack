...
theorem compile_full_pan_value_call_state_raised_of_source_compiled_call
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
  ...
  obtain ⟨compiledValues, targetParameters, targetBody, targetCalleeLocals,
      ...
      htarget⟩ := evalCrepFullCallState_none_raised_inversion
    functions crepPrimitive ffi sharedMem
    baseAddress topAddress targetFuel caller function compiledArguments target
    crepException hcrepCall
  obtain ⟨sourceArgumentValues, sourceParameters, sourceBody,
  ...
  have hnamesDeclaration : (declaration.params.map Prod.fst).Nodup := by
    rw [hparams]
    exact hnames sourceParameters sourceBody hlookupSourceDecl
  have hnamesCompiled :
      ((compileParamVars declaration.params 0).1.map Prod.fst).Nodup := by
    have hnamesEq := congrArg (List.map Prod.fst)
      (compileParamVars_preserves_parameter_shapes declaration.params 0)
