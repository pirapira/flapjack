...
theorem compile_full_pan_value_call_state_relation
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
  ...
  have hresult := compile_full_pan_value_call_state_compose
    context structs sourceFunctions functions
    sourceLocals sourceGlobals sourceMemory state
    primitive sourceHandler crepPrimitive ffi sharedMem
    baseAddress topAddress bytesInWord fuel
...
/-! The caught-handler path can retain the callee's compiler-owned payload
spill after the handler returns.  The exact-memory relation above is therefore
too strong for that boundary; this companion keeps the excluded addresses
explicit while retaining the same source/Crep evaluator composition. -/
theorem compile_full_pan_value_call_relation_except
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    ...
    (hcrepCall : evalCrepFullCall functions crepPrimitive ffi sharedMem
      baseAddress topAddress fuel state compiledInfo function compiledArguments =
      some crepResult)
    (hrel : panValueCrepControlRelExcept structs context exceptionRel
      sourceResult crepResult excluded) :
    evalPanValueProgWithPrimitiveCallsAndFfi
      primitive sourceHandler structs sourceFunctions
      baseAddress topAddress bytesInWord (fuel + 1)
      sourceLocals sourceGlobals sourceMemory
    ...
    evalCrepFullProg functions crepPrimitive ffi sharedMem
      baseAddress topAddress (fuel + 1) state
      (compileProg context (.call info function arguments)) = some crepResult ∧
    panValueCrepControlRelExcept structs context
      exceptionRel sourceResult crepResult excluded := by
  have hresult := compile_full_pan_value_call_compose
    context structs sourceFunctions functions
    sourceLocals sourceGlobals sourceMemory state
    primitive sourceHandler crepPrimitive ffi sharedMem
...
theorem compile_full_pan_value_call_state_compose_except
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    ...
    evalCrepFullProgState functions crepPrimitive ffi sharedMem
      baseAddress topAddress (fuel + 1) state
      (compileProg context (.call info function arguments)) = some crepResult ∧
    panValueCrepControlRelExcept structs context exceptionRel
      sourceResult crepResult excluded := by
  have hresult := compile_full_pan_value_call_state_compose
    context structs sourceFunctions functions
    sourceLocals sourceGlobals sourceMemory state
    primitive sourceHandler crepPrimitive ffi sharedMem
...
theorem compile_full_pan_value_call_state_relation_except
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    ...
    evalCrepFullProgState functions crepPrimitive ffi sharedMem
      baseAddress topAddress (fuel + 1) state
      (compileProg context (.call info function arguments)) = some crepResult ∧
    panValueCrepControlRelExcept structs context exceptionRel sourceResult
      crepResult excluded := by
  exact compile_full_pan_value_call_state_compose_except
    context structs sourceFunctions functions
    sourceLocals sourceGlobals sourceMemory state
    primitive sourceHandler crepPrimitive ffi sharedMem
    baseAddress topAddress bytesInWord fuel
    info compiledInfo function arguments compiledArguments
    sourceResult crepResult exceptionRel excluded
    hcompileArgs hcompileProg hsourceCall hcrepCall hrel
end Flapjack
