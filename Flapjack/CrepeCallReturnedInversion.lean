...
theorem evalCrepFullCallState_none_returned_inversion
    [BEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
              ...
              | some calleeLocals =>
                  ...
                  | some calleeResult =>
                      ...
                      | returned callee calleeValues =>
                          have hsome :
                              some (CrepControlResult.returned
                                (CrepState.mk caller.locals callee.memory callee.globals) calleeValues) =
                                some (CrepControlResult.returned target targetValues) := by
                          ...
                          · exact hcallee
                          · have htarget :
                                some (CrepState.mk caller.locals callee.memory callee.globals) = some target := by
                              simpa [evalCrepFullCallState, hvalues, hlookup, hassign, hcallee]
                                using hcall
                            exact (Option.some.inj htarget).symm
                      | normal callee
