...
theorem evalCrepFullProgState_call_seq_inversion
    [BEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    ...
    ∃ callResult,
      ...
      | .normal callState =>
          evalCrepFullProgState functions primitive ffi sharedMem
            baseAddress topAddress (fuel + 1) callState body = some result
      | other => result = other := by
  cases hcallResult : evalCrepFullCallState functions primitive ffi sharedMem
      baseAddress topAddress fuel state info function arguments with
  | none =>
      simp [evalCrepFullProgState, hcallResult] at heval
  | some callResult =>
      refine ⟨callResult, rfl, ?_⟩
      cases callResult with
      | normal callState =>
          simpa [evalCrepFullProgState, hcallResult] using heval
      | returned callState values =>
          have hresult :
              some (.returned callState values) = some result := by
            simpa [evalCrepFullProgState, hcallResult] using heval
