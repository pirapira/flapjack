...
/-! Stateful conditional composition. -/

theorem compile_full_pan_value_ite_then_state_relation
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    ...
    (baseAddress topAddress bytesInWord : α) (fuel : Nat)
    (condition : Exp α) (compiledCondition : CrepExp α)
    (thenBranch elseBranch : Prog α) (compiledThen compiledElse : CrepProg α)
    (sourceCondition targetCondition : α)
    (sourceThenResult : PanValueControlResult α)
    (crepThenResult : CrepControlResult α)
    (exceptionRel : ExceptionId → PanValue α → α → Prop)
    (hcompileCondition : compileExp context condition = ([compiledCondition], .one))
    (hcompileThen : compileProg context thenBranch = compiledThen)
    (hcompileElse : compileProg context elseBranch = compiledElse)
    (hsourceCondition : evalPanValueExp structs sourceLocals sourceGlobals
      sourceMemory baseAddress topAddress bytesInWord condition =
    ...
    evalPanValueProgWithPrimitiveCallsAndFfi primitive sourceHandler structs
      ...
      sourceGlobals sourceMemory (.ite condition thenBranch elseBranch) =
      some sourceThenResult ∧
    evalCrepFullProgState functions crepPrimitive ffi sharedMem
      baseAddress topAddress (fuel + 1) state
      (compileProg context (.ite condition thenBranch elseBranch)) =
      some crepThenResult ∧
    panValueCrepControlRel structs context exceptionRel sourceThenResult
      crepThenResult := by
  have hcompile : compileProg context (.ite condition thenBranch elseBranch) =
      .ite compiledCondition compiledThen compiledElse := by
    simp [compileProg, hcompileCondition, hcompileThen, hcompileElse]
  ...
  · simp [evalPanValueProgWithPrimitiveCallsAndFfi, hsourceCondition,
      hnonzero, hsourceThen]
  · constructor
    · simp [evalCrepFullProgState, hcrepCondition, hconditionAgreement,
        hnonzero, hcrepThen]
    · exact hthenRel

theorem compile_full_pan_value_ite_else_state_relation
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    ...
    evalCrepFullProgState functions crepPrimitive ffi sharedMem
      ...
      (compileProg context (.ite condition thenBranch elseBranch)) =
      some crepElseResult ∧
    panValueCrepControlRel structs context exceptionRel sourceElseResult
      crepElseResult := by
  have hsourceZero : sourceCondition = 0 := by simpa using hzero
  have htargetZero : targetCondition = 0 := by
    simpa [hconditionAgreement] using hsourceZero
  have hcompile : compileProg context (.ite condition thenBranch elseBranch) =
      .ite compiledCondition compiledThen compiledElse := by
  ...
  · simp [evalPanValueProgWithPrimitiveCallsAndFfi, hsourceCondition,
      hsourceZero, hsourceElse]
  · constructor
    · simp [evalCrepFullProgState, hcrepCondition, htargetZero, hcrepElse]
    · exact helseRel
