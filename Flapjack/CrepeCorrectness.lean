...
theorem compile_full_pan_value_ite_word_const_correct
  ...
  · simp [compileProg, compileExp, evalCrepFullResultState,
      evalCrepFullProgState, evalCrepFullExpState, evalCrepFullExpsState,
      evalPanValueProg, evalPanValueProgWithPrimitive, evalPanValueExp,
...
theorem compile_full_pan_value_ite_compose
  ...
  rw [hcompile]
  simp only [evalCrepFullResultState, evalCrepFullProgState, hcompiledCondition,
    evalPanValueProg, evalPanValueProgWithPrimitive, hsourceCondition]
  by_cases hcondition : (conditionValue != 0) = true
  · simp [hcondition]
    simpa [evalCrepFullResultState, evalPanValueProg] using hthen
  · simp [hcondition]
    simpa [evalCrepFullResultState, evalPanValueProg] using helse
...
theorem compile_full_pan_value_dec_word_return_correct
  ...
  simp [compileProg, compileExp, allocatedNames, nestedDecs,
    ...
    evalCrepFullExpState, evalPanValueProg, evalPanValueProgWithPrimitive,
...
/-! Stateful counterpart of the nonzero/break loop rule.  The source witness
    and break consumption are unchanged; the target uses the complete state
    evaluator so the post-body locals, globals, and memory are preserved. -/
theorem compile_full_pan_value_while_break_compose_state_full
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [PanShiftWidth α]
    [ArithmeticShiftRight α] [RotateRightOp α]
  ...
  · simp [evalPanValueProgWithPrimitiveCallsAndFfi, hsourceCondition,
      hsourceNonzero, hsourceBody]
  · simp [compileProg, hcompileCondition, hcompileBody,
      evalCrepFullProgStateFull, hcrepCondition, hconditionAgreement,
      hsourceNonzero, hcrepBody]
...
/-! Global-aware form of the shared-memory store boundary.  The temporary
    compiler local is restored after the handler transition while the
    handler's memory and global state are retained. -/
theorem compile_full_pan_value_shMemStore_word_state_correct
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
...
/-! Global-aware form of the two-word raise boundary.  Structured payload
    words are spilled in source order into the explicit Crep global state. -/
theorem compile_full_pan_value_raise_two_word_state_correct
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
