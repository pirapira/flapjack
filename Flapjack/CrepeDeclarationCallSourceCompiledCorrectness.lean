...
theorem compile_full_pan_value_decCall_state_returned_of_source_compiled_call
  ...
  obtain ⟨sourceArgumentValues, sourceParameters, sourceCalleeBody,
      ...
      _, hbind, hsourceCallee, _, _⟩ := evalPanValueCall_returned_inversion
    primitive sourceHandler structs sourceFunctions
    sourceLocals sourceGlobals sourceMemory baseAddress topAddress bytesInWord sourceCallFuel
    none none none function arguments sourceCalleeGlobals sourceCalleeMemory [sourceValue]
    hsourceCall
  have hlookupSourceDecl : lookupPanFunction function
      (sourceFunctionEntries declarations) = some (sourceParameters, sourceCalleeBody) := by
  ...
  have hparametersLength : sourceParameters.length = sourceArgumentValues.length :=
    hbindExpanded.1
  have hcallState := evalCrepFullCallState_normal_inversion
    functions crepPrimitive ffi sharedMem
    baseAddress topAddress targetCallFuel
    { state with
        locals := initializeCrepLocals state.locals
          (allocatedNames context shape) }
    (allocatedNames context shape) function compiledArguments callState hcrepCall
  cases hcallState with
  | inl hnormal =>
      obtain ⟨targetArgumentValues, targetParameters, targetCalleeBody,
        targetCalleeLocals, targetCallee, htargetValues, hlookupCompiled,
        hassign, hcrepCalleeBody, htarget⟩ := hnormal
      have hcompiledValues' : targetArgumentValues =
          sourceArgumentValues.flatMap panValueFlatWords := by
        exact hargumentValues sourceArgumentValues targetArgumentValues
          hsourceArguments htargetValues
      have hassignSource : assignCrepValues (fun _ => none) targetParameters
        ...
        rw [← hcompiledValues']
        exact hassign
      obtain ⟨declaration, hnameDeclaration, hparams, hbodyDeclaration,
          htargetParameters, htargetBody, hcompileBody, hstate⟩ :=
        sourceCompiledCalleeState_of_lookup
          structs functionContext declarations functions function sourceParameters
          sourceCalleeBody sourceGlobals sourceMemory state.memory sourceArgumentValues
          targetParameters targetCalleeBody sourceCalleeLocals targetCalleeLocals
          hlookupSourceDecl hfunctions hparametersLength hglobals hmemory hfunctionContext
          (hnames sourceParameters sourceCalleeBody hlookupSourceDecl)
          (fun parameters returnShape =>
            hshape parameters returnShape sourceArgumentValues)
          (fun parameters returnShape =>
      ...
      have hbodyCorrect : PanValueCrepProgramStateCorrect sourceCalleeBody := by
        ...
        rw [hbodyDeclaration] at h
        exact h
      have hcompileBody' : compileProg calleeContext sourceCalleeBody = targetCalleeBody := by
        rw [← hbodyDeclaration]
      ...
      have hcrepBody' : evalCrepFullProgState functions crepPrimitive ffi sharedMem
          ...
          (compileProg calleeContext sourceCalleeBody) =
          some (.normal targetCallee) := by
        rw [hcompileBody']
        exact hcrepCalleeBody
      have hbodyRel := hbodyCorrect calleeContext structs sourceFunctions functions
        ...
        baseAddress topAddress bytesInWord sourceCallFuel targetCallFuel exceptionRel
        (.returned sourceBodyLocals sourceCalleeGlobals sourceCalleeMemory [sourceValue])
        (.normal targetCallee) hstate' hsourceCallee hcrepBody'
      simp [panValueCrepControlRel] at hbodyRel
  | inr hreturned =>
      obtain ⟨targetArgumentValues, targetParameters, targetCalleeBody,
        targetCalleeLocals, targetCallee, targetCalleeValues, targetCallerLocals,
        htargetValues, hlookupCompiled, hassign, hcrepCalleeBody, hdestinations,
        htarget⟩ := hreturned
      have hcompiledValues' : targetArgumentValues =
          sourceArgumentValues.flatMap panValueFlatWords := by
        exact hargumentValues sourceArgumentValues targetArgumentValues
      ...
      have hcompileBody' : compileProg calleeContext sourceCalleeBody = targetCalleeBody := by
        rw [← hbodyDeclaration]
        simpa [calleeContext] using hcompileBody
      have hcrepCalleeBody' : evalCrepFullProgState functions crepPrimitive ffi sharedMem
          baseAddress topAddress targetCallFuel
          { locals := targetCalleeLocals, memory := state.memory,
            globals := state.globals }
          (compileProg calleeContext sourceCalleeBody) =
      ...
      have hcrepBody' := hcrepBody
      rw [htarget] at hcrepBody'
      have hresult := compile_full_pan_value_decCall_state_returned_of_body_correct
        context structs sourceFunctions functions sourceLocals sourceGlobals sourceMemory state
        primitive sourceHandler crepPrimitive ffi sharedMem
        baseAddress topAddress bytesInWord sourceCallFuel targetCallFuel exceptionRel
        name shape function calleeContext arguments body sourceCalleeBody targetCalleeBody
        compiledArguments sourceCalleeLocals sourceCalleeGlobals sourceCalleeMemory
