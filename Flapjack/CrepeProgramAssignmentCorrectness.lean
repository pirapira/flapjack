...
theorem evalCrepFullExpsState_length
    [BEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    ...
    (h : evalCrepFullExpsState state baseAddress topAddress expressions =
      some values) :
    expressions.length = values.length := by
  induction expressions generalizing values with
  | nil =>
      cases values with
      | nil => rfl
      | cons value values => simp [evalCrepFullExpsState] at h
  | cons expression expressions ih =>
      cases values with
      | nil =>
          cases hhead : evalCrepFullExpState state baseAddress topAddress expression with
          | none => simp [evalCrepFullExpsState, hhead] at h
          | some headValue =>
              cases htail : evalCrepFullExpsState state baseAddress topAddress expressions with
              | none => simp [evalCrepFullExpsState, hhead, htail] at h
              | some tailValues => simp [evalCrepFullExpsState, hhead, htail] at h
      | cons value values =>
          cases hhead : evalCrepFullExpState state baseAddress topAddress expression with
          | none => simp [evalCrepFullExpsState, hhead] at h
          | some headValue =>
              ...
              | some tailValues =>
                  have htailLength := ih tailValues htail
                  have hvalues : headValue :: tailValues = value :: values := by
                    simpa [evalCrepFullExpsState, hhead, htail] using h
                  cases hvalues
                  simp [htailLength]
...
theorem panValueCrepProgramStateCorrect_assign_local_of_expression_contract
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
  ...
  have hcontract : ∀ (context : CompileContext α) (structs : StructContext)
    ...
    intro context structs sourceLocals sourceGlobals sourceMemory state
      baseAddress topAddress bytesInWord sourceValue hrel hsource
    obtain ⟨_, compiled, hcompile, hcompiled⟩ := hvalue
      context structs sourceLocals sourceGlobals sourceMemory state
      baseAddress topAddress bytesInWord sourceValue hrel hsource
    let values := panValueFlatWords sourceValue
    refine ⟨compiled, panValueShape structs sourceValue, values, hcompile, rfl, ?_, ?_, rfl⟩
    · exact evalCrepFullExpsState_length state baseAddress topAddress compiled values
        (by simpa [values] using hcompiled)
    · simpa [values] using hcompiled
  obtain ⟨compiled, compileShape, values, hcompile, hcompileShape,
      hcompiledLength, hcompiled, hflat⟩ := hcontract context structs sourceLocals
    sourceGlobals sourceMemory state baseAddress topAddress bytesInWord sourceValue
    hrel hsourceValue
  obtain ⟨shape, slots, hlookupName⟩ := hlookup context sourceLocals oldValue hold
  have holdRel := hrel.2.1 name oldValue shape slots hold hlookupName
  have hshapeNewContext : panShapeMatches
  ...
  have htemporaryPath' : ∀ (context : CompileContext α) (structs : StructContext)
    ...
    · intro compiledExpression hcompiledExpression varName hvar
      ...
      exact List.mem_flatMap.2 ⟨compiledExpression, hcompiledExpression, hvar⟩
  by_cases hdirectMode : distinctLists slots (compiled.flatMap crepExpVars) = true
  · exact panValueCrepProgramStateCorrect_assign_local_direct name expression hcontract
      hdirect hmetadata hlookup hnoalias context structs sourceFunctions functions
      sourceLocals sourceGlobals sourceMemory state primitive sourceHandler crepPrimitive
      ffi sharedMem baseAddress topAddress bytesInWord sourceFuel targetFuel exceptionRel
      sourceResult crepResult hrel hsource hcrep
  · exact panValueCrepProgramStateCorrect_assign_local_temporary name expression hcontract
      htemporaryPath' hmetadata hbounded hlookup hnoalias context structs sourceFunctions
