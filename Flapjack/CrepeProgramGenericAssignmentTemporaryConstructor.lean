...
theorem panValueCrepProgramCorrect_assign_local_temporary
  ...
  have hreadTemporary := readCrepLocals_updateCrepLocalList_of_not_mem
    ...
    (fun temporary htemporary hslot => hnoOverlap temporary htemporary hslot)
...
theorem panValueCrepProgramStateCorrect_assign_local_temporary
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
  ...
  have hslotBound : ∀ slot ∈ slots, slot ≤ context.maxVar :=
    hbounded context name shape slots hlookupName
  have hcompiledFresh := freshNames_not_mem_of_expVars_bounded context
    slots.length 1 compiled (by omega) hcompiledBounded
  have hnoOverlap := freshNames_not_mem_of_bounded context slots.length 1 slots
    (by omega) hslotBound
  let temporarySlots := freshNames context slots.length 1
  have htemporaryLength : temporarySlots.length = values.length := by
  ...
  have hbodyEval := evalCrepFullExpsState_varList_updateCrepLocalList
    state baseAddress topAddress temporarySlots values htemporaryLength
    htemporaryDistinct
