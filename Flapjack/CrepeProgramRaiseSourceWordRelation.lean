...
/-! The same scalar raise lowering through the global-aware full evaluator.
    Unlike the compatibility theorem above, this conclusion records the
    reserved payload in `CrepState.globals`. -/
set_option linter.unusedSimpArgs false in
theorem compile_full_pan_value_raise_source_word_global_relation_fuel
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    ...
    panValueCrepRaisedGlobalSpillRel structs context sourceGlobals sourceMemory
      (.word value) value
      { state with globals := updateMemory state.globals 0 value } 0 ∧
    exceptionRel exception (.word value) exceptionCode := by
  have hcompileProg : compileProg context (.raise exception expression.toExp) =
      .seq
        (nestedDecs [context.maxVar + 1] [compiled]
          (crepNestedSeq (storeGlobals 0 context.bytesInWord
  ...
  · have hrestore : restoreCrepLocal
    ...
    simp [nestedDecs, crepNestedSeq, storeGlobals,
      ...
      hcompiled, hfresh, updateCrepLocal, restoreCrepResult, hrestore]
  · exact ⟨panValueCrepRaisedGlobalSpillRel_word
      structs context sourceLocals sourceGlobals sourceMemory state 0 value hrel,
      hexception⟩
