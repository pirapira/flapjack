...
/-!
Program-level correctness for a two-word record declaration whose body
continues.  The declaration's temporary bindings are restored while the
continue result is propagated through the source and Crep control boundaries.
-/

namespace Flapjack

set_option linter.unusedSimpArgs false in
theorem panValueCrepProgramCorrect_dec_two_word_record_continue
    ...
    PanValueCrepProgramCorrect
      (.dec name (.comb [.one, .one])
        (.rStruct [left.toExp, right.toExp]) .continue) := by
          ...
          | some rightValue' =>
              ...
              have hsourceExpected :
                  evalPanValueProgWithPrimitiveCallsAndFfi
                    ...
                    (.dec name (.comb [.one, .one])
                      (.rStruct [left.toExp, right.toExp]) .continue) =
                    some (.continued sourceLocals sourceGlobals sourceMemory) := by
                cases sourceFuel with
                | zero =>
                    simp [evalPanValueProgWithPrimitiveCallsAndFfi] at hsource
                | succ sourceFuel =>
              ...
              have hcompileProg :
                  compileProg context
                    ...
                    nestedDecs [context.maxVar + 1, context.maxVar + 2]
                      [compiledLeft, compiledRight] (.continue 0) := by
                      ...
                      | succ targetFuel =>
                          ...
                          have hcontrol :
                              panValueCrepControlRel structs context exceptionRel
                                ...
                                (.continued state 0) := by
                            simpa [panValueCrepControlRel] using hrel
                          rw [hsourceEq, hcrepEq] at hcontrol
                          exact hcontrol


theorem panValueCrepProgramStateCorrect_dec_two_word_record_continue
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
