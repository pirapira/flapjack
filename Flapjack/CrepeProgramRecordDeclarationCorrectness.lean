...
/-!
Program-level correctness for a two-word record declaration returning its
bound value.  This is the first declaration constructor in the compositional
correctness predicate: source values retain their record shape while Crep
materializes the two words in fresh locals and restores them on return.
-/

namespace Flapjack

set_option linter.unusedSimpArgs false in
theorem panValueCrepProgramCorrect_dec_two_word_record_return
  ...
  | succ sourceFuel =>
      ...
      | some leftValue' =>
          ...
          | some rightValue' =>
              ...
              let returnedLocals :=
                restorePanValueLocal (fun _ => none) name (sourceLocals name)
              have hsourceExpected :
                  evalPanValueProgWithPrimitiveCallsAndFfi
                    ...
                    some (.returned returnedLocals sourceGlobals sourceMemory
                      [.rStruct [.word leftValue, .word rightValue]]) := by
                cases sourceFuel with
                | zero =>
                    simp [evalPanValueProgWithPrimitiveCallsAndFfi] at hsource
                | succ sourceFuel =>
                    simp [evalPanValueProgWithPrimitiveCallsAndFfi,
                      ...
                      restorePanValueControlLocal, returnedLocals,
                      updatePanValueMap]
              ...
              have hcompileProg :
                  compileProg context
                    ...
                    nestedDecs [context.maxVar + 1, context.maxVar + 2]
                      ...
                      (.return [.var (context.maxVar + 1),
                        .var (context.maxVar + 2)]) := by
              ...
              | succ targetFuel =>
                  ...
                  | succ targetFuel =>
                      ...
                      | succ targetFuel =>
                              ...
                              evalCrepFullProg functions crepPrimitive ffi
                                  ...
                                  (compileProg context
                                    (.dec name (.comb [.one, .one])
                                      (.rStruct [left.toExp, right.toExp])
                                      (.return (.var .local name)))) =
                                some (.returned state [leftValue, rightValue]) := by
                            rw [hcompileProg]
                          ...
                          have hreturnedRel :
                              panValueCrepStateRel structs context returnedLocals
                                sourceGlobals sourceMemory state := by
                            refine ⟨hrel.1, ?_, hrel.2.2⟩
                            intro current currentValue shape slots hcurrent hlookupCurrent
                            by_cases hname : current == name
                            · have heq : current = name := by simpa using hname
                              subst current
                              have hold : sourceLocals name = some currentValue := by
                                simpa [returnedLocals, restorePanValueLocal] using hcurrent
                              exact hrel.2.1 name currentValue shape slots hold hlookupCurrent
                            · simp [returnedLocals, restorePanValueLocal, hname] at hcurrent
                          have hcontrol :
                              panValueCrepControlRel structs context exceptionRel
                                (.returned returnedLocals sourceGlobals sourceMemory
                                  [.rStruct [.word leftValue, .word rightValue]])
                                (.returned state [leftValue, rightValue]) := by
                            refine ⟨hreturnedRel, ?_⟩
...
set_option linter.unusedSimpArgs false in
theorem panValueCrepProgramStateCorrect_dec_two_word_record_return
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
                  ...
                  | succ targetFuel =>
                      ...
                      | zero =>
                          simp [nestedDecs, evalCrepFullProgState,
                            evalCrepFullExpsState] at hcrep
                      | succ targetFuel =>
                          have htargetExpected :
                            ...
                            simp [nestedDecs, evalCrepFullProgState, evalCrepFullExpState,
                              evalCrepFullExpsState, hcrepLeft, hcrepRightAfter,
                              updateCrepLocal, restoreCrepResult, hrestore,
                              Nat.add_assoc]
                          have hsourceEq :=
                            Option.some.inj (hsourceExpected.symm.trans hsource)
