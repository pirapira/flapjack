...
/-!
Program-level correctness for a two-word record declaration with a normal
`skip` body.  The declaration evaluates and materializes both source words,
then both evaluators restore the state that preceded the declaration.
-/

namespace Flapjack

set_option linter.unusedSimpArgs false in
theorem panValueCrepProgramCorrect_dec_two_word_record_skip
    ...
    PanValueCrepProgramCorrect
      (.dec name (.comb [.one, .one])
        (.rStruct [left.toExp, right.toExp]) .skip) := by
          ...
          | some rightValue' =>
              ...
              have hsourceExpected :
                  evalPanValueProgWithPrimitiveCallsAndFfi
                    ...
                    (.dec name (.comb [.one, .one])
                      (.rStruct [left.toExp, right.toExp]) .skip) =
...
theorem panValueCrepProgramStateCorrect_dec_two_word_record_skip
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
    [ShiftLeft α] [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
