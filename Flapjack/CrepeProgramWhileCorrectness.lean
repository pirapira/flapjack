...
def PanValueCrepProgramLoopStateControlSafe
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α] [ShiftLeft α] [ShiftRight α]
    [LT α] [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    (program : Prog α) : Prop :=
  ∀ (context : CompileContext α) (structs : StructContext)
    ...
    | .continued _ _ _, .continued _ label => label = 0
    | _, _ => True
theorem panValueCrepProgramStateCorrect_while_source_word_pos
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α] [ShiftLeft α] [ShiftRight α]
    [LT α] [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    (condition : SourceWordExp α) (body : Prog α)
          ...
          | word sourceCondition =>
              obtain ⟨compiledCondition, hcompileCondition, hcrepCondition⟩ :=
                compileSourceWordExp_state_relation context structs sourceLocals
                  ...
                  condition sourceCondition hcondition
              by_cases hzero : sourceCondition = 0
              · have hzeroResult := compile_full_pan_value_while_source_word_zero_state_relation_fuel
                  context structs sourceFunctions functions sourceLocals sourceGlobals
                  sourceMemory state primitive sourceHandler crepPrimitive ffi sharedMem
                  baseAddress topAddress bytesInWord 0 targetFuel condition body
                  (compileProg context body) sourceCondition sourceCondition exceptionRel
          ...
          | word sourceCondition =>
              ...
              · cases targetFuel with
                | zero =>
                    have hnonzero' : (sourceCondition == 0) = false := by
                      simpa using hzero
                    simp [evalCrepFullProgState, hcrepCondition, hnonzero'] at hcrep
                | succ targetFuel =>
                    have hnonzero : sourceCondition ≠ 0 := hzero
                    have hnonzero' : (sourceCondition == 0) = false := by
                      simpa using hnonzero
                    ...
                    | some sourceBodyResult =>
                        cases hcrepBody :
                            evalCrepFullProgState functions crepPrimitive ffi sharedMem
                              ...
                              (compileProg context body) with
                        | none =>
                            simp [evalCrepFullProgState, hcrepCondition, hnonzero',
                              hcrepBody] at hcrep
                        | some crepBodyResult =>
                            have hsourceBody' :
                                evalPanValueProgWithPrimitiveCallsAndFfi
                                ...
                                | normal nextState =>
                                    ...
                                    have hcrepLoop :
                                        evalCrepFullProgState functions crepPrimitive ffi sharedMem
                                          ...
                                          (.while compiledCondition (compileProg context body)) =
                                          some crepResult := by
                                      simpa [evalCrepFullProgState, hcrepCondition,
                                        hnonzero', hcrepBody] using hcrep
                                    have hcrepLoop' :
                                        evalCrepFullProgState functions crepPrimitive ffi sharedMem
                                          baseAddress topAddress (targetFuel + 1) nextState
                                    ...
                                    have hloopRel := ih nextLocals nextGlobals nextMemory nextState
                                      targetFuel sourceResult crepResult hnextRel
                                      hsourceLoop hcrepLoop'
                                    have hstep := compile_full_pan_value_while_nonzero_compose_state_fuel
                                      context structs sourceFunctions functions sourceLocals
                                      sourceGlobals sourceMemory nextLocals nextGlobals
                                      nextMemory state nextState primitive sourceHandler
                                      crepPrimitive ffi sharedMem baseAddress topAddress
                                ...
                                | broke nextState label =>
                                    ...
                                    have hnextRel :
                                        panValueCrepStateRel structs context nextLocals
                                          nextGlobals nextMemory nextState := by
                                      simpa [panValueCrepControlRel] using hbodyRel
                                    have hstep := compile_full_pan_value_while_break_compose_state_fuel
                                      context structs sourceFunctions functions sourceLocals
                                      sourceGlobals sourceMemory nextLocals nextGlobals
                                      nextMemory state nextState primitive sourceHandler
                                      crepPrimitive ffi sharedMem baseAddress topAddress
                            ...
                            | continued nextLocals nextGlobals nextMemory =>
                                ...
                                | continued nextState label =>
                                    ...
                                    have hloopRel := ih nextLocals nextGlobals nextMemory nextState
                                      targetFuel sourceResult crepResult hnextRel
                                      hsourceLoop hcrepLoop'
                                    have hstep := compile_full_pan_value_while_continue_compose_state_fuel
                                      context structs sourceFunctions functions sourceLocals
                                      sourceGlobals sourceMemory nextLocals nextGlobals
                                      nextMemory state nextState primitive sourceHandler
                                      crepPrimitive ffi sharedMem baseAddress topAddress
...
theorem panValueCrepProgramStateCorrect_while_source_word
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α] [ShiftLeft α] [ShiftRight α]
    [LT α] [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    (condition : SourceWordExp α) (body : Prog α)
    ...
    (hlookup : ∀ (context : CompileContext α)
      ...
      sourceLocals name = some value →
      ∃ slot, lookupInfo name context.vars = some (.one, [slot])) :
    PanValueCrepProgramStateCorrect (.while condition.toExp body) := by
  intro context structs sourceFunctions functions sourceLocals sourceGlobals
    sourceMemory state primitive sourceHandler crepPrimitive ffi sharedMem
    baseAddress topAddress bytesInWord sourceFuel targetFuel exceptionRel
    sourceResult crepResult hrel hsource hcrep
  ...
  | succ sourceFuel =>
      ...
      | succ targetFuel =>
          exact panValueCrepProgramStateCorrect_while_source_word_pos condition body
            hbody hbodySafe context structs sourceFunctions functions sourceLocals
            sourceGlobals sourceMemory state primitive sourceHandler crepPrimitive
            ffi sharedMem baseAddress topAddress bytesInWord sourceFuel targetFuel
            exceptionRel sourceResult crepResult (hbytesInWord context bytesInWord)
            (hlookup context) hrel hsource hcrep
