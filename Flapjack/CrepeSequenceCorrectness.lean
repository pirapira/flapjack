...
/-! Target-word version of the mixed-fuel sequence rule.  This is the same
    composition theorem as above, but its Crep side uses the complete Cake
    `word_sh` evaluator.  Keeping the extra word operations in the theorem
    boundary prevents a RISC-V caller from silently falling back to the
    partial compatibility evaluator. -/
theorem compile_full_pan_value_seq_normal_relation_mixed_fuel_full
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α]
  ...
  · simp [evalPanValueProgWithPrimitiveCallsAndFfi, hsourceFirst,
      hsourceSecond]
  · constructor
    · simp [evalCrepFullProgStateFull, hcrepFirst, hcrepSecond]
    · exact hsecondRel
