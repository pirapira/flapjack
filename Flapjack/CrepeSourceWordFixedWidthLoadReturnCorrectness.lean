...
theorem compile_full_pan_value_return_load32_state_relation
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α] [ShiftLeft α] [ShiftRight α]
    [LT α] [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    (context : CompileContext α) (structs : StructContext)
    ...
    panValueCrepControlRel structs context exceptionRel
      (.returned (fun _ => none) sourceGlobals sourceMemory [.word value])
      (.returned state [value]) := by
  obtain ⟨compiled, hcompile, hcompiled⟩ := compileSourceWord_load32_state_relation
    context structs sourceLocals sourceGlobals sourceMemory state baseAddress topAddress
    bytesInWord address addressValue value hbytesInWord hlocals hlookup
    hsourceAddress hsource
  have hcompileShape : compileExp context (.load32 address.toExp) =
      ([compiled], panValueShape structs (.word value)) := by
    simpa [panValueShape] using hcompile
  ...
  exact compile_full_pan_value_return_state_relation_fuel context structs
    ...
    bytesInWord 0 0 (.load32 address.toExp) (.word value) [compiled] exceptionRel
    hsource (panValuePayloadWithinLimit_word structs value) hcompileShape
    hcompiledList hlocals

theorem compile_full_pan_value_return_loadByte_state_relation
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α] [ShiftLeft α] [ShiftRight α]
    [LT α] [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    (context : CompileContext α) (structs : StructContext)
    ...
    panValueCrepControlRel structs context exceptionRel
      (.returned (fun _ => none) sourceGlobals sourceMemory [.word value])
      (.returned state [value]) := by
  obtain ⟨compiled, hcompile, hcompiled⟩ := compileSourceWord_loadByte_state_relation
    context structs sourceLocals sourceGlobals sourceMemory state baseAddress topAddress
    bytesInWord address addressValue value hbytesInWord hlocals hlookup
    hsourceAddress hsource
  have hcompileShape : compileExp context (.loadByte address.toExp) =
  ...
  exact compile_full_pan_value_return_state_relation_fuel context structs
    sourceFunctions functions sourceLocals sourceGlobals sourceMemory state
    primitive sourceHandler crepPrimitive ffi sharedMem baseAddress topAddress
    bytesInWord 0 0 (.loadByte address.toExp) (.word value) [compiled] exceptionRel
    hsource (panValuePayloadWithinLimit_word structs value) hcompileShape
    hcompiledList hlocals
