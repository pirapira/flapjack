...
/-! Compose a source-word StoreByte with a one-word load return. -/

namespace Flapjack

theorem compile_full_pan_value_storeByte_load_one_source_word_relation
    ...
    evalPanValueProgWithPrimitiveCallsAndFfi primitive sourceHandler structs
      sourceFunctions baseAddress topAddress bytesInWord 2 sourceLocals sourceGlobals
      sourceMemory (.seq (.storeByte storeAddress.toExp storeValue.toExp)
  ...
  obtain ⟨loadSource, loadCrep, loadRel⟩ :=
    compile_full_pan_value_return_load_one_relation context structs sourceFunctions
  ...
  have hsequence := compile_full_pan_value_seq_normal_relation context structs
    ...
    (.storeByte storeAddress.toExp storeValue.toExp)
    (.return (.load .one loadAddress.toExp))
    (compileProg context (.storeByte storeAddress.toExp storeValue.toExp))
    ...
    (.returned
      { state with memory := updateMemory state.memory storeAddressValue storeValueValue }
      [loadValue]) exceptionRel rfl rfl storeSource storeCrepEval loadSource loadCrep
    loadRel
