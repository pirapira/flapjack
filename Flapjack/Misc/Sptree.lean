...
/-! ## HOL `fromAList` and its lookup theorem
...
The CakeML theorem `mem_lookup_fromalist_some` is ported and tagged below over
this exact `Spt` carrier. -/
...
/-- Exact port of CakeML `mem_lookup_fromalist_some`
    ...
    association-list recursion uses exact `Spt` insertion and no list-lookup
    substitute. -/
