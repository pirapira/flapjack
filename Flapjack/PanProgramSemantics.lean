...
/-! The other singleton-return branch is rejected when the entry's declared
    return shape does not accept the value.  This is the failure branch of
    Cake's top-level `evaluate_decls` boundary, rather than a failure hidden
    inside the callee evaluator. -/
theorem evalPanValueProgram_of_declarations_and_returned_shape_mismatch
    [BEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α] [ShiftLeft α] [ShiftRight α]
    [LT α] [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    (initial : PanValueProgramState α)
    ...
    (hcall : evalPanValueCallWithPrimitiveCallsAndFfi primitive ffi
      ...
      some (.returned locals globals memory [value]))
    (hentry : lookupInfo entry state.returnShapes = some shape)
    (hshape : panShapeMatches (panValueShape state.structs value) shape = false) :
    evalPanValueProgram initial primitive ffi fuel declarations entry arguments
      (memoryAccess := memoryAccess) (memoryHandler := memoryHandler) = none := by
  simp [evalPanValueProgram, hdeclarations, hcall, hentry, hshape]
