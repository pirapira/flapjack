...
/-! Manual well-founded induction for Cake's `exp_ids_ret_to_tail_eq`.  As
    with `seqAssoc`, call handlers are nested syntax and need an explicit
    induction hypothesis. -/

theorem expIds_retToTail (program : Prog α) :
    expIds (retToTail program) = expIds program := by
  let rec go : (program : Prog α) → expIds (retToTail program) = expIds program
    | .skip => by
