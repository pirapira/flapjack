...
/-! Constructor-level counterpart of Cake's `pc_compile_correct[Assign]`.
    The expression/state relation is intentionally explicit: local-word
    assignment has a stronger convenience theorem above, while this branch
    is the general assembly point for both local and global assignments. -/
set_option linter.unusedVariables false in
theorem panValuePcCompileCorrect_compact_assign_of_state_evidence
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α] [ShiftLeft α]
    [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    ...
    (baseAddress topAddress bytesInWord : α)
    (sourceFuel targetFuel : Nat)
    (hprogram : PanValueCrepProgramStateCorrect (.assign kind name value))
    (hpost : ∀ (context : CompileContext α) (structs : StructContext)
      (exceptionRel : ExceptionId → PanValue α → α → Prop)
      (sourceLocals sourceGlobals : VarName → Option (PanValue α))
      (sourceMemory : α → Option (PanValue α)) (sourceException : ExceptionId)
    ...
    PanValuePcCompileCorrect
      ...
      (crepPcCompactTargetEvaluator functions crepPrimitive ffi sharedMem
        baseAddress topAddress targetFuel)
      codeRel excpRel exceptionCode globalsLookup (.assign kind name value) := by
  exact panValuePcCompileCorrect_compact_with_raised_state_evidence
    (.assign kind name value) codeRel excpRel exceptionCode globalsLookup
    sourceFunctions functions primitive sourceHandler crepPrimitive ffi sharedMem
    baseAddress topAddress bytesInWord sourceFuel targetFuel hprogram
    (panValueCrepProgramStateControlSafe_assign kind name value)
