...
/-! Context-coded counterpart of the constructor assembly.  Unlike the
    ordinary projection, this keeps the source/target evaluator code and
    exception-shape evidence in the induction predicate itself. -/
set_option linter.unusedVariables false in
theorem panValuePcCompileCorrectWithContextCode_compact_of_constructor_induction
    [BEq α] [LawfulBEq α] [OfNat α 0] [OfNat α 1] [Add α] [Mul α]
    [Sub α] [AndOp α] [OrOp α] [HXor α α α] [ShiftLeft α]
    [ShiftRight α] [LT α]
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    ...
    (exceptionCode : ExceptionId → Option α)
    (globalsLookup : CrepState α → PanValue α → Option (List α))
    (hskip : PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
      codeRel excpRel exceptionCode globalsLookup (.skip : Prog α))
    (hdec : ∀ (name : VarName) (shape : Shape) (value : Exp α)
      (body : Prog α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup body →
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.dec name shape value body))
    (hassign : ∀ (kind : VarKind) (name : VarName) (value : Exp α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.assign kind name value))
    (hprimitive : ∀ (name : VarName) (operator : PrimOp) (args : List (Exp α)),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.primitive name operator args))
    (hstore : ∀ (address value : Exp α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.store address value))
    (hstore32 : ∀ (address value : Exp α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.store32 address value))
    (hstoreByte : ∀ (address value : Exp α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.storeByte address value))
    (hseq : ∀ (first second : Prog α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup first →
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup second →
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.seq first second))
    (hite : ∀ (condition : Exp α) (thenBranch elseBranch : Prog α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup thenBranch →
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup elseBranch →
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.ite condition thenBranch elseBranch))
    (hwhile : ∀ (condition : Exp α) (body : Prog α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup body →
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.while condition body))
    (hbreak : PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
      codeRel excpRel exceptionCode globalsLookup (.break : Prog α))
    (hcontinue : PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
    ...
    (hcall : ∀
      ...
      (match info with
       | some (_, some (_, _, handler)) =>
           PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
             codeRel excpRel exceptionCode globalsLookup handler
       | _ => True) →
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.call info name args))
    (hdecCall : ∀ (name : VarName) (shape : Shape) (function : FunName)
      (args : List (Exp α)) (body : Prog α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup body →
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.decCall name shape function args body))
    (hextCall : ∀ (function : FunName)
      (configuration configurationLength array arrayLength : Exp α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
    ...
    (hraise : ∀ (exception : ExceptionId) (value : Exp α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.raise exception value))
    (hreturn : ∀ (value : Exp α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.return value))
    (hshMemLoad : ∀ (size : OpSize) (kind : VarKind) (name : VarName)
      (address : Exp α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.shMemLoad size kind name address))
    (hshMemStore : ∀ (size : OpSize) (address value : Exp α),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (.shMemStore size address value))
    (htick : PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
      codeRel excpRel exceptionCode globalsLookup (.tick : Prog α))
    (hannot : ∀ (tag text : String),
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup (@Prog.annot α tag text)) :
    ∀ program : Prog α,
      PanValuePcCompileCorrectWithContextCode sourceEvaluate targetEvaluate
        codeRel excpRel exceptionCode globalsLookup program := by
  exact panValueProg_compile_correct_induction
    (P := fun program => PanValuePcCompileCorrectWithContextCode sourceEvaluate
