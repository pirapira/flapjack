...
/-!
# Compact `pc_compile_correct` assembly for primitive statements

This is the primitive-constructor counterpart of the declaration and
assignment wrappers.  The detailed state simulation remains a separate
premise, as in CakeML's constructor proof: once it is supplied, the source
primitive's control-safety leaf closes the result-label obligation at the
...
theorem panValuePcCompileCorrect_compact_primitive_of_state_evidence
  ...
  exact panValuePcCompileCorrect_compact_with_raised_state_evidence
    (.primitive name operator arguments) codeRel excpRel exceptionCode
    ...
    hprogram (panValueCrepProgramStateControlSafe_primitive name operator arguments)
