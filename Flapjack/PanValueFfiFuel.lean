...
This module supplies it for Flapjack's public stepped stateful-FFI evaluator:
...
The clocked evaluator is intentionally a separate follow-up boundary; this
module covers the unclocked evaluator used by the current source contracts.
-/
...
def CallMono (context : PanValueFfiContext α) (primitive : PanPrimitiveHandler α)
    (handler : PanValueStatefulFfiHandler α σ) (structs : StructContext)
    (functions : List (FunName × List VarName × Prog α))
...
def ProgMono (context : PanValueFfiContext α) (primitive : PanPrimitiveHandler α)
  ...
  ∀ fuel' result, fuel ≤ fuel' →
    evalPanValueFfiProgSteps context primitive handler structs functions
      ...
      (memoryAccess := ma) (contracts := c) (memoryHandler := mh) = some result →
    evalPanValueFfiProgSteps context primitive handler structs functions
      baseAddress topAddress bytesInWord fuel' locals globals memory ffi program
      (memoryAccess := ma) (contracts := c) (memoryHandler := mh) = some result
...
theorem call_succ_mono (context : PanValueFfiContext α) (primitive : PanPrimitiveHandler α)
    (handler : PanValueStatefulFfiHandler α σ) (structs : StructContext)
    (functions : List (FunName × List VarName × Prog α))
    (baseAddress topAddress bytesInWord : α)
    ...
    | some p =>
      ...
      | normal l g m f =>
        ...
        cases hs : evalPanValueFfiProgSteps context primitive handler structs functions
            baseAddress topAddress bytesInWord fuel l g m f second
            (memoryAccess := ma) (contracts := c) (memoryHandler := mh) with
      ...
      | returned l g m f vs =>
        ...
        | cons value rest =>
          ...
          | nil =>
            ...
            · rw [if_pos hshape] at h ⊢
              cases hb : evalPanValueFfiProgSteps context primitive handler structs functions
                baseAddress topAddress bytesInWord fuel (updatePanValueMap locals name value) g m f body
                (memoryAccess := ma) (contracts := c) (memoryHandler := mh) with
              | none => rw [hb] at h; simp at h
              | some q =>
                rw [hb] at h
...
theorem callMono (context : PanValueFfiContext α) (primitive : PanPrimitiveHandler α)
    (handler : PanValueStatefulFfiHandler α σ) (structs : StructContext)
    (functions : List (FunName × List VarName × Prog α))
    (baseAddress topAddress bytesInWord : α) :
    ∀ (fuel : Nat) (locals globals : VarName → Option (PanValue α))
...
theorem evalPanValueFfiProgramStepped_fuel_mono
  ...
  | some state =>
    ...
    cases hcall : evalPanValueFfiCallSteps context primitive handler state.structs
        ...
        (some (PanValueCallContracts.mk state.returnShapes state.exceptions
