...
/-! ### `pan_simp` preserves byte-rangedness

`DeclByteRanged`/`ProgByteRanged` (bead flapjack-pxn.18.3.5.8.7.1.3) are purely
structural: every identifier is a `String` whose characters fit in HOL's
256-character universe and every embedded shape/expression is byte-ranged.  The
`pan_simp` rewrites below only reassociate sequences, turn an assign/return
wrapper into a tail call, and splice call handlers; they never introduce a name
or an expression, so they preserve both predicates. -/
...
theorem retToTail_byteRanged {width : Nat} (program : Prog (BitVec width))
  ...
  let rec go : (program : Prog (BitVec width)) → ProgByteRanged program →
    ...
    | .call (some (kindOpt, none)) function arguments, hbody => by simpa [retToTail] using hbody
    | .call (some (kindOpt, some (eid, vn, handlerProgram))) function arguments, hbody => by
        simp only [retToTail]
        simp only [ProgByteRanged] at hbody ⊢
        exact ⟨hbody.1, hbody.2.1, hbody.2.2.1, hbody.2.2.2.1,
    ...
    | .decCall name shape function arguments body, hbody => by
        simp only [retToTail]
        exact ⟨hbody.1, hbody.2.1, hbody.2.2.1, hbody.2.2.2.1, go body hbody.2.2.2.2⟩
