...
/-- Derived RStruct induction case for HOL `compile_exp_correct`.
    ...
    induction step; each hypothesis retains the outer context, finite-map,
    field-validity, and struct-info premises. This case proves all three
    conclusions using production `structCompileExps` and `evalPanValueExps`.
    It is untagged because it specializes the universal HOL theorem through
    Lean's total-lookup state adapter and explicit `bytesInWord` argument. -/
theorem panStructCompileExpCorrectRStructCase
    ...
    (context : StructPassContext) (state : PanSemState α ffi)
    (bytesInWord : α) (expressions : List (Exp α)) (value : PanValue α)
    (heval : evalPanValueExp state.structs state.locals state.globals
      state.memory state.baseAddress state.topAddress bytesInWord
      (.rStruct expressions) = some value)
  ...
  cases hvalues : evalPanValueExp.evalPanValueExps state.structs state.locals
      state.globals state.memory state.baseAddress state.topAddress bytesInWord expressions with
  | none => simp [evalPanValueExp, hvalues] at heval
  | some values =>
      have hvalue : PanValue.rStruct values = value := by
        simpa [evalPanValueExp, hvalues] using heval
      cases hvalue
      have hsourceValues : evalPanValueExps state.structs state.locals state.globals
          state.memory state.baseAddress state.topAddress bytesInWord expressions = some values := by
        simpa [evalPanValueExps] using hvalues
      have hshapeFields := panStructRStructListShapeFields context state.structs
        state.locals state.globals state.memory state.baseAddress state.topAddress
        bytesInWord expressions hfieldsIH values hsourceValues
      ...
      have htargetList := panStructCompileExpsEvalOfPointwiseCorrect
        context state bytesInWord expressions values hsourceValues hpointwise
      ...
      · simpa [evalPanValueExp, structCompileExp, panStructConvertState,
...
/-- Derived NStruct-constructor case of HOL `compile_exp_correct`, with all
    ...
    expression. It is intentionally untagged: HOL has one universally
    quantified theorem, not a separately named NStruct declaration, and this
    Lean statement is translated rather than exact. Lean replaces the direct
    `ctxt.structs = MAP ... s.structs` premise with projected shape-view
    equality, HOL finite-map `FEVERY` validity with pointwise Bool predicates
    over total lookups, and `FMAP_MAP2` equations with pointwise shape-map
    adapters. `structInfosOk` is retained and used to establish declared-field
    uniqueness. The recursive hypotheses carry the translated premises and
    all three conclusions. The production evaluator uses `PanSemState` total
    lookups and an explicit `bytesInWord` parameter instead of HOL finite maps
    and its fixed-width evaluator. -/
theorem panStructCompileExpCorrectNStructCase
    ...
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    (context : StructPassContext) (state : PanSemState α ffi)
    (bytesInWord : α) (name : StructName) (fields : List (FieldName × Exp α))
    (value : PanValue α)
    (heval : evalPanValueExp state.structs state.locals state.globals state.memory
      state.baseAddress state.topAddress bytesInWord (.nStruct name fields) = some value)
    (hstructs : panStructContextShapeView context.structs =
      panStructContextShapeView state.structs)
    (hlocalsFields : panStructEveryValueFieldsOkBool state.structs state.locals)
    (hglobalsFields : panStructEveryValueFieldsOkBool state.structs state.globals)
  ...
  | none =>
      simp [evalPanValueExp, hlookup] at heval
  | some info =>
      cases hfieldEval : evalPanValueExp.evalPanValueFields state.structs
          state.locals state.globals state.memory state.baseAddress state.topAddress
          bytesInWord fields with
      | none =>
          simp [evalPanValueExp, hlookup, hfieldEval] at heval
      | some fieldValues =>
          by_cases hshapes : panValueFieldsHaveShapes state.structs info.fields
              fieldValues = true
          · have hvalue : value = .nStruct name fieldValues := by
              simp [evalPanValueExp, hlookup, hfieldEval, hshapes] at heval
              exact heval.symm
            subst value
            ...
            have hprojection := evalPanValueFields_projection state.structs
              state.locals state.globals state.memory state.baseAddress state.topAddress
              bytesInWord fields fieldValues hfieldEval
            have hsourceValues : evalPanValueExps state.structs state.locals
                state.globals state.memory state.baseAddress state.topAddress bytesInWord
                (fields.map Prod.snd) = some (fieldValues.map Prod.snd) :=
              hprojection.2
            have hfieldIH : PanStructAll (fun expression => ∀ subvalue,
              ...
              exact panStructAll_map (fun expression ih => fun subvalue hevalSub =>
                let hcase := ih subvalue hevalSub hstructs hlocalsFields hglobalsFields
                  hstructInfos hlocalsMap hglobalsMap
                ⟨hcase.1, hcase.2.1⟩) hinduction
            have hfieldFacts := panStructRStructListShapeFields context state.structs
              state.locals state.globals state.memory state.baseAddress state.topAddress
              bytesInWord (fields.map Prod.snd) hfieldIH (fieldValues.map Prod.snd)
              hsourceValues
            ...
            have hfieldNamesExprs : info.fields.map Prod.fst = fields.map Prod.fst := by
              calc
                ...
                _ = fields.map Prod.fst := hprojection.1.symm
            ...
            have hpointwise : ∀ expression, expression ∈ fields.map Prod.snd → ∀ subvalue,
              ...
              exact (panStructAll_mem hinduction expression hmem subvalue hevalSub
                hstructs hlocalsFields hglobalsFields hstructInfos hlocalsMap hglobalsMap).2.2
            have htargetList := panStructCompileExpsEvalOfPointwiseCorrect
              context state bytesInWord (fields.map Prod.snd) (fieldValues.map Prod.snd)
            ...
            have htargetListLocal :
              ...
              simpa only [evalPanValueExps] using htargetList
...
/-- Derived NField-constructor case of HOL `compile_exp_correct`. The
    ...
    conclusions. Field lookup is connected to the production compiler's
    `structFindFieldIndex` by preserving the declared and runtime field order.
    This specialization is intentionally untagged: HOL has one universal
    theorem, while Lean uses projected struct-shape equality, Bool validity
    over total lookups, and an explicit `bytesInWord` parameter. The
...
theorem panStructCompileExpCorrectNFieldCase
    ...
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    (context : StructPassContext) (state : PanSemState α ffi)
    (bytesInWord : α) (field : FieldName) (expression : Exp α)
    (value : PanValue α)
    (heval : evalPanValueExp state.structs state.locals state.globals state.memory
      state.baseAddress state.topAddress bytesInWord (.nField field expression) = some value)
    (hstructs : panStructContextShapeView context.structs =
      panStructContextShapeView state.structs)
    (hlocalsFields : panStructEveryValueFieldsOkBool state.structs state.locals)
    (hglobalsFields : panStructEveryValueFieldsOkBool state.structs state.globals)
  ...
  | some childValue =>
      ...
      | rStruct values => simp [evalPanValueExp, hchild] at heval
      | nStruct structName fields =>
          ...
          | none => simp [evalPanValueExp, hchild, hlookupStruct] at heval
          | some info =>
              cases hfield : lookupPanValueField field fields with
              | none => simp [evalPanValueExp, hchild, hlookupStruct, hfield] at heval
              | some fieldValue =>
                  have hvalue : value = fieldValue := by
                    have hval : fieldValue = value := by
                      simp [evalPanValueExp, hchild, hlookupStruct, hfield] at heval
                      exact heval
                    exact hval.symm
                  subst value
...
/-- Derived Load32-constructor specialization of HOL `compile_exp_correct`.
    It keeps the address induction hypothesis and proves all three conclusion
    roles, but remains untagged: HOL `Load32` reads and endian-combines four
    bytes through finite memory, while this production evaluator's default
    `evalPanValueExp` branch reads one word from a total memory lookup (the
    optional `read32` callback is not passed here). The explicit
    `bytesInWord` and translated context/FEVERY/FMAP_MAP2 interfaces are also
    different from HOL. This theorem is only for successful executions of the
    Lean evaluator and does not claim the Load32 evaluator is a faithful HOL
    memory port. -/
theorem panStructCompileExpCorrectLoad32Case
    ...
    (hinduction : ∀ addressValue,
      ...
      evalPanValueExp (panStructConvertState context state).structs
        ...
        (structCompileExp context addressExpression) =
          some (panStructConvertValue addressValue)) :
    structOldExpShape context (.load32 addressExpression) = panSemShapeOf value ∧
    panStructValueFieldsOkBool state.structs value = true ∧
    evalPanValueExp (panStructConvertState context state).structs
      (panStructConvertState context state).locals
      (panStructConvertState context state).globals
                    ...
                    have hconvertedLoad :
                        evalPanValueExp []
                          ...
                          (.load32 (structCompileExp context addressExpression)) =
                            some (.word loadedWord) := by
                      simp [evalPanValueExp, hcompiledAddress, hmemory]
...
/-- Derived BaseAddr-constructor specialization of HOL `compile_exp_correct`.
    ...
    retained; the literal case does not use them. Lean's context view and
    total-function Bool/map adapters differ from HOL's finite-map statements. -/
theorem panStructCompileExpCorrectBaseAddrCase
    ...
    (heval : evalPanValueExp state.structs state.locals state.globals state.memory
      state.baseAddress state.topAddress bytesInWord .baseAddr = some value)
    (_hstructs : panStructContextShapeView context.structs =
      panStructContextShapeView state.structs)
    (_hlocalsFields : panStructEveryValueFieldsOkBool state.structs state.locals)
    (_hglobalsFields : panStructEveryValueFieldsOkBool state.structs state.globals)
    ...
    evalPanValueExp (panStructConvertState context state).structs
      ...
      (panStructConvertState context state).baseAddress
      (panStructConvertState context state).topAddress bytesInWord
      (structCompileExp context .baseAddr) = some (panStructConvertValue value) := by
  cases value with
  | word wordValue =>
...
/-- Derived TopAddr-constructor specialization of HOL `compile_exp_correct`.
    ...
    BaseAddr case; HOL has no separately named TopAddr theorem. -/
theorem panStructCompileExpCorrectTopAddrCase
    ...
    (bytesInWord : α) (value : PanValue α)
    (heval : evalPanValueExp state.structs state.locals state.globals state.memory
      state.baseAddress state.topAddress bytesInWord .topAddr = some value)
    (_hstructs : panStructContextShapeView context.structs =
      panStructContextShapeView state.structs)
    (_hlocalsFields : panStructEveryValueFieldsOkBool state.structs state.locals)
    (_hglobalsFields : panStructEveryValueFieldsOkBool state.structs state.globals)
    ...
    evalPanValueExp (panStructConvertState context state).structs
      ...
      (panStructConvertState context state).baseAddress
      (panStructConvertState context state).topAddress bytesInWord
      (structCompileExp context .topAddr) = some (panStructConvertValue value) := by
  cases value with
  | word wordValue =>
...
/-! Op-constructor specialization of HOL `compile_exp_correct`. It is
    intentionally untagged: HOL states one universally quantified theorem and
    has no separately named Op-case declaration. This Lean specialization
    retains successful source evaluation, the translated context/state,
    field-validity, `struct_infos_ok`, and local/global shape-map premises in
    every argument induction hypothesis. As in the other production cases,
    the direct HOL finite-map equalities are represented by a shape view,
    pointwise Bool predicates, and the `PanSemState` lookup interface; Lean
    also takes an explicit `bytesInWord`. The Op evaluator filters evaluated
    operands to words and accepts exactly two, so the proof explicitly carries
    the same value conversion through that filter. -/
theorem panStructCompileExpCorrectOpCase
    ...
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    (context : StructPassContext) (state : PanSemState α ffi)
    (bytesInWord : α) (operator : BinOp) (arguments : List (Exp α))
    (value : PanValue α)
    (heval : evalPanValueExp state.structs state.locals state.globals state.memory
      state.baseAddress state.topAddress bytesInWord (.op operator arguments) = some value)
    (hstructs : panStructContextShapeView context.structs =
      panStructContextShapeView state.structs)
    (hlocalsFields : panStructEveryValueFieldsOkBool state.structs state.locals)
    (hglobalsFields : panStructEveryValueFieldsOkBool state.structs state.globals)
  ...
  have hmapWordConvert : ∀ operands : List (PanValue α),
    ...
    | cons operand operands ih =>
        simp [List.mapM_cons, hwordConvert operand, ih]
  have hmapWordConvertCompose : ∀ operands : List (PanValue α),
      operands.mapM (fun operand => asWord (panStructConvertValue operand)) =
        (operands.map panStructConvertValue).mapM asWord := by
    intro operands
    induction operands with
    | nil => rfl
    | cons operand operands ih => simp [List.mapM_cons, ih]
  ...
  | some operands =>
      have harguments' : evalPanValueExp.evalPanValueExps state.structs
        ...
        simpa [evalPanValueExps] using harguments
      cases hwords : operands.mapM asWord with
      | none =>
          have hevalAfterArguments := heval
          simp only [evalPanValueExp, harguments'] at hevalAfterArguments
          change (operands.mapM asWord).bind (fun words =>
            match words with
            | [left, right] => some (PanValue.word (evalPanBinOp operator left right))
            | _ => none) = some value at hevalAfterArguments
          rw [hwords] at hevalAfterArguments
          simp at hevalAfterArguments
      | some words =>
          have hevalAfterArguments := heval
          simp only [evalPanValueExp, harguments'] at hevalAfterArguments
          change (operands.mapM asWord).bind (fun words =>
          ...
          | cons left rest =>
              ...
              | cons right rest =>
                  ...
                  | cons extra rest =>
                      ...
                      simp at hevalAfterArguments
                  | nil =>
                      have hvalue : value = .word (evalPanBinOp operator left right) := by
                        rw [hwords] at hevalAfterArguments
                        simp only [Option.bind_some] at hevalAfterArguments
                        exact (Option.some.inj hevalAfterArguments).symm
                      have hcompiledArgs := panStructCompileExpsEvalOfPointwiseCorrect
                        context state bytesInWord arguments operands harguments hpointwise
                      have hcompiledArgs' : evalPanValueExp.evalPanValueExps
                          (panStructConvertState context state).structs
                          (panStructConvertState context state).locals
                          (panStructConvertState context state).globals
                          (panStructConvertState context state).memory
                          ...
                          (structCompileExp.structCompileExps context arguments) =
                            some (operands.map panStructConvertValue) := by
                        simpa [evalPanValueExps] using hcompiledArgs
                      have htargetWords :
                          (operands.map panStructConvertValue).mapM asWord = some [left, right] := by
                        rw [hmapWordConvert, hwords]
                      have htargetWordsInline :
                          (operands.map panStructConvertValue).mapM
                            (fun (operand : PanValue α) => match operand with
                              | PanValue.word word => some word
                              | _ => none) = some [left, right] := by
                        simpa [asWord] using htargetWords
                      have htargetWordsOp : operands.mapM
                          (fun operand => asWord (panStructConvertValue operand)) =
                            some [left, right] := by
                        rw [hmapWordConvertCompose, htargetWords]
                      have htargetWordsComposed : operands.mapM
                          (((fun (operand : PanValue α) => match operand with
                            | PanValue.word word => some word
                            | _ => none) : PanValue α → Option α) ∘
                            panStructConvertValue) = some [left, right] := by
                        change operands.mapM (fun operand =>
                          asWord (panStructConvertValue operand)) = some [left, right]
                        exact htargetWordsOp
                      refine ⟨?_, ?_, ?_⟩
                      ...
                      · have hcompiledOp : structCompileExp context (.op operator arguments) =
                        ...
                        rw [hcompiledOp]
                        rw [hvalue]
                        simp [evalPanValueExp, hcompiledArgs', Option.bind]
                        generalize htargetMap :
                          List.mapM (((fun (operand : PanValue α) =>
                            match operand with
                            | PanValue.word word => some word
                            | _ => none) : PanValue α → Option α) ∘
                            panStructConvertValue) operands = targetWords at *
                        have htargetWordsEq : targetWords = some [left, right] := by
...
/-- Derived binary Cmp-constructor specialization of HOL
    ...
    It remains untagged: HOL has only the universal theorem, while this Lean
    interface uses projected context views, total lookup/Bool adapters, and
    explicit `bytesInWord`. -/
theorem panStructCompileExpCorrectCmpCase
    ...
    [DecidableRel (fun left right : α => left < right)] [PanCmp α]
    (context : StructPassContext) (state : PanSemState α ffi)
    (bytesInWord : α) (operator : Cmp) (left right : Exp α)
    (value : PanValue α)
    (heval : evalPanValueExp state.structs state.locals state.globals state.memory
      state.baseAddress state.topAddress bytesInWord (.cmp operator left right) = some value)
    (hstructs : panStructContextShapeView context.structs =
      panStructContextShapeView state.structs)
    (hlocalsFields : panStructEveryValueFieldsOkBool state.structs state.locals)
    (hglobalsFields : panStructEveryValueFieldsOkBool state.structs state.globals)
  ...
  | none => simp [evalPanValueExp, hleft] at heval
  | some leftValue =>
      cases leftValue with
      | rStruct fields => simp [evalPanValueExp, hleft] at heval
      | nStruct name fields => simp [evalPanValueExp, hleft] at heval
      | word leftWord =>
          ...
          | none => simp [evalPanValueExp, hleft, hright] at heval
          | some rightValue =>
              cases rightValue with
              | rStruct fields => simp [evalPanValueExp, hleft, hright] at heval
              | nStruct name fields => simp [evalPanValueExp, hleft, hright] at heval
              | word rightWord =>
                  have hvalue : value = .word (evalPanCmp operator leftWord rightWord) := by
                    have hsource := heval
...
/-- Derived binary Shift-constructor specialization of HOL
    `compile_exp_correct`. The left and right recursive hypotheses retain the
    translated context, FEVERY, FMAP_MAP2, and `struct_infos_ok` premise roles
    and all three conclusions. Successful Shift evaluation forces word
    operands, and compiling preserves the same `evalPanShift` result. It
    remains untagged: HOL has only the universal theorem, while this Lean
    interface uses projected context views, total lookup/Bool adapters, and
    explicit `bytesInWord`. The compact production `evalPanShift` currently
    implements LSL/LSR and returns `none` for HOL's ASR/ROR cases; this case
    evaluator also does not reject nonzero shift amounts at or above the HOL
    word width. This case proves the transported theorem for successful
    executions of that Lean evaluator, not a claim that the evaluator itself
    is a complete HOL port. -/
  ...
  | some leftValue =>
      ...
      | word leftWord =>
          ...
          | some rightValue =>
              ...
              | nStruct name fields => simp [evalPanValueExp, hleft, hright] at heval
              | word rightWord =>
                  cases hshift : evalPanShift operator leftWord rightWord with
                  | none => simp [evalPanValueExp, hleft, hright, hshift] at heval
                  | some result =>
                      have hvalue : value = .word result := by
                        have hsource := heval
                        simp [evalPanValueExp, hleft, hright, hshift] at hsource
                        exact hsource.symm
                      have hleftIH := hinductionLeft (.word leftWord) hleft
                        hstructs hlocalsFields hglobalsFields hstructInfos hlocalsMap hglobalsMap
                      have hrightIH := hinductionRight (.word rightWord) hright
