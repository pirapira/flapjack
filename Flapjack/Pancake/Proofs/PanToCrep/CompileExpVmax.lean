...
/-! Source review of HOL `eval_var_cexp_present_ctxt`
    ...
    `EvalFinite.lean` now tags `evalHOLFinite` as `eval_def` over
    `PanSemStateFiniteExact` with `fmap_as_finite_support` (integration commit
    `71b9dac42`), so the exact evaluator prerequisite has landed. It does not
    port Pan-to-Crep's `state_rel`, `code_rel`, or `locals_rel`. The available
    cross-pass `stateRel` in `PanToCrep.lean` is
