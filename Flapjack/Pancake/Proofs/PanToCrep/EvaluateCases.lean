...
/-- Source-state execution of a one-word exception caught by a matching
Call handler. The handler local must already contain a word, as required by
HOL `is_valid_value`; the caught payload overwrites it before the handler
returns that local. This is untagged support for the
`pc_compile_correct[Call_Ret_Exception]` case. -/
theorem panSemEvaluateRiscV64CodeState_callHandlesRaiseOneWordException_ofEntry
    (context : PanValueFfiContext (RiscV.Word 64))
    (primitive : PanPrimitiveHandler (RiscV.Word 64))
    (handler : PanValueStatefulFfiHandler (RiscV.Word 64) σ)
    (state : PanSemState (RiscV.Word 64) (FfiState σ))
    (function exception handlerVariable : String)
    (value oldValue : RiscV.Word 64)
    (hentry : panSemCodeLookup state.code function =
      some ([], .raise exception (.const value), .one))
    (hexception : state.exceptionShapes exception = some .one)
    ...
    panSemEvaluateRiscV64CodeState context primitive handler state
      ...
      some (.control (.returned
        (fun _ => none)
        state.globals state.memory state.ffi [.word value]), decPanClock state.clock) := by
  let program : Prog (RiscV.Word 64) :=
    .call (some (none, some (exception, handlerVariable,
      .return (.var .local handlerVariable)))) function []
  have hargs : evalPanValueExps state.structs state.locals state.globals
  ...
  have hcallee : lookupPanSemCodeCall state.structs state.code function [] =
    ...
    simp [panSemCodeArgumentsMatch, bindPanValueParameters]
  have hhandlerValue :
      panValueAssignmentValid state.structs state.locals (fun _ => none)
        .local handlerVariable (.word value) = true := by
    simp [panValueAssignmentValid, hhandlerLocal, panValueShape, panShapeMatches]
  have hpayload : panValuePayloadWithinLimit state.structs (.word value) = true := by
    simp
  have hfuelLower : 3 ≤ panSemCodeEvaluateFuel state program := by
    unfold panSemCodeEvaluateFuel
    have hclockLower : 2 ≤ state.clock + 1 := by
      have hclockPos : 1 ≤ state.clock := Nat.one_le_iff_ne_zero.mpr hclock
      omega
    have hprogramFuel : 2 ≤ panSemProgFuel program := by
      simp [program, panSemProgFuel, panSemCallInfoFuel,
        panSemExpListFuel, panSemExpFuel]
    have hbodyLower : 3 ≤
        max (panSemProgFuel program) (panSemCodeBodyFuel state.code) + 1 := by
      have hmax : 2 ≤ max (panSemProgFuel program)
          (panSemCodeBodyFuel state.code) :=
        Nat.le_trans hprogramFuel (Nat.le_max_left _ _)
      omega
    have hmul := Nat.mul_le_mul hclockLower hbodyLower
    omega
  obtain ⟨tail, hfuel⟩ := Nat.exists_eq_add_of_le hfuelLower
  have hfuelConcrete : panSemCodeEvaluateFuel state program = tail + 3 := by
    omega
  unfold panSemEvaluateRiscV64CodeState panSemEvaluateCodeStateWithMemoryModel
  unfold panSemEvaluateCodeState panSemEvaluateCodeStateWithFuel
  rw [hfuelConcrete]
  cases tail <;>
    simp [evalPanValueFfiClockCodeProg, evalPanValueFfiClockCodeCall,
      evalPanValueFfiClockLeaf, evalPanValueFfiProgSteps,
      evalPanValueExpCounted, evalPanValueExp, hargs, hcallee,
      hexception, hhandlerValue, hpayload, hclock,
      panValueShape, panShapeMatches,
      updatePanValueMap, beq_iff_eq, decPanClock]
...
/-! Actual-state matching Call exception-handler case for HOL
`pc_compile_correct[Call_Ret_Exception]`. The source handler local is an
existing one-word local, as required by HOL's `is_valid_value`; `localsRel`
derives its target slot. `excpRel` supplies the target exception code, and
`codeRel` supplies the compiled callee. The handler's compiled `loadGlob 0`
reads the word published by the callee through `globals_lookup`. As in HOL's
Return result branch, the conclusion has no post-state `localsRel` clause. -/
theorem panToCrepPcCompileCorrectCallHandlesOneWordExceptionCodeStateRiscV64
    (context : PanToCrepProofContext (RiscV.Word 64))
    (sourceContext : PanValueFfiContext (RiscV.Word 64))
    (sourcePrimitive : PanPrimitiveHandler (RiscV.Word 64))
    (sourceHandler : PanValueStatefulFfiHandler (RiscV.Word 64) σ)
    ...
    (hentry : panSemCodeLookup sourceState.code function =
      some ([], .raise exception (.const value), .one))
    (hexception : sourceState.exceptionShapes exception = some .one)
    (hhandlerLocal : sourceState.locals handlerVariable = some (.word oldValue))
    (hclock : sourceState.clock ≠ 0) :
    let handlerProgram : Prog (RiscV.Word 64) :=
      .return (.var .local handlerVariable)
    let program : Prog (RiscV.Word 64) :=
      .call (some (none, some (exception, handlerVariable, handlerProgram))) function []
    let sourceResult : PanValueFfiClockResult (RiscV.Word 64) σ :=
      (.control (.returned (fun _ => none) sourceState.globals sourceState.memory
        sourceState.ffi [.word value]), decPanClock sourceState.clock)
    panSemEvaluateRiscV64CodeState sourceContext sourcePrimitive sourceHandler
      sourceState program = some sourceResult ∧
    ∃ targetPost,
      evalCrepRuntimeResult targetHandler targetPrimitive 12 targetState
  ...
  rcases hstate with ⟨hmem, hmemaddrs, hshared, hstructs, hglobals,
    hclockRel, hbe, hffi, hbase, htop⟩
  have hsourceHandlerLookup : FLOOKUP sourceState.locals handlerVariable =
      some (.word oldValue) := by
    simpa [FLOOKUP] using hhandlerLocal
  obtain ⟨handlerSlots, hcontextHandler, hhandlerSlotLength,
      hhandlerWords, _hhandlerWf⟩ :=
    localsRelLookupCtxt context sourceState.locals targetState.locals
      handlerVariable (.word oldValue) hlocals hsourceHandlerLookup
  obtain ⟨handlerSlot, hhandlerSlots⟩ : ∃ slot, handlerSlots = [slot] := by
    cases handlerSlots with
    | nil => simp [panValueFlatten] at hhandlerSlotLength
    | cons slot rest =>
        cases rest with
        | nil => exact ⟨slot, rfl⟩
        | cons next rest => simp [panValueFlatten] at hhandlerSlotLength
  have hcontextHandlerOne :
      FLOOKUP context.vars handlerVariable = some (.one, [handlerSlot]) := by
    simpa [panValueShape, hhandlerSlots] using hcontextHandler
  have htargetHandlerLocal : targetState.locals handlerSlot =
      some (.word oldValue) := by
    rw [hhandlerSlots] at hhandlerWords
    cases hlookup : targetState.locals handlerSlot with
    | none => simp_all [FLOOKUP, panValueFlatten]
    | some cell =>
        have hcell : cell = PanWordLab.word oldValue := by
          simp [FLOOKUP, hlookup, panValueFlatten] at hhandlerWords
          exact hhandlerWords
        simp [hcell]
  have hcontextException : ∃ exceptionCode,
      context.eids exception = some exceptionCode := by
    have hsourceDomain : sourceState.exceptionShapes exception ≠ none := by
      rw [hexception]
      exact Option.some_ne_none _
    have htargetDomain : context.eids exception ≠ none := by
      have hdomain := congrFun hexcp.1 exception
      change (sourceState.exceptionShapes exception ≠ none) =
        (context.eids exception ≠ none) at hdomain
      rw [hexception] at hdomain
      exact Eq.mp hdomain (Option.some_ne_none _)
    cases hvalue : context.eids exception with
    | none => exact False.elim (htargetDomain hvalue)
    | some exceptionCode => exact ⟨exceptionCode, rfl⟩
  obtain ⟨exceptionCode, hcontextException⟩ := hcontextException
  have hcontextExceptionLookup : FLOOKUP context.eids exception =
      some exceptionCode := by
    change context.eids exception = some exceptionCode
    exact hcontextException
  let targetBody : CrepProg (RiscV.Word 64) :=
    .seq (.dec 1 (.const value)
      (.seq (.storeGlob (0 : BitVec 5) (.var 1)) .skip))
      (.raise exceptionCode)
  ...
  have hcodeEntry := codeRelImp context
    (panSemCodeAsLookup sourceState.code) targetState.code hcode function []
    (.raise exception (.const value)) .one hsourceLookup
  rcases hcodeEntry with ⟨_hlocalized, hfunctionContext, htargetLookup⟩
  have hcompiledBody : compileCodeRelProg functionContext
      (.raise exception (.const value)) = targetBody := by
    simp [functionContext, targetBody, compileCodeRelProg, compileProgHOL,
      compileExpHOL, freshNamesHOL, ctxtFc, maxList, hcontextExceptionLookup,
      nestedDecs, crepNestedSeq, storeGlobals]
  have htargetCode : FLOOKUP targetState.code function = some ([], targetBody) := by
    simpa [Shape.shapeSize, functionContext, maxList, hcompiledBody] using
      htargetLookup
  have htargetCallLookup : lookupCrepRuntimeCode function [] targetState.code =
      some (targetBody, fun _ => none) := by
    unfold lookupCrepRuntimeCode
  ...
  have hsource : panSemEvaluateRiscV64CodeState sourceContext sourcePrimitive
      sourceHandler sourceState program = some sourceResult := by
    simpa [program, handlerProgram, sourceResult] using
      panSemEvaluateRiscV64CodeState_callHandlesRaiseOneWordException_ofEntry
        sourceContext sourcePrimitive sourceHandler sourceState function
        exception handlerVariable value oldValue hentry hexception
        hhandlerLocal hclock
  have htargetClock : targetState.clock ≠ 0 := by omega
  let returnSlot := context.vmax + 1
  let handlerContinuation : CrepProg (RiscV.Word 64) :=
    .seq (.seq (.assign handlerSlot (.loadGlob (0 : BitVec 5))) .skip)
      (.return [.var handlerSlot])
  let targetProgram : CrepProg (RiscV.Word 64) :=
    .dec returnSlot (.const 0)
      (.call (some ([returnSlot], some (exceptionCode, handlerContinuation)))
        function [])
  let targetPost : CrepRuntimeState (RiscV.Word 64) σ :=
    { decCrepClock targetState with
      ...
      globals := updateCrepRuntimeGlobal targetState.globals
        (0 : BitVec 5) (.word value) }
  have hhandlerSlotBound : handlerSlot ≤ context.vmax := by
    have hbound := ctxtMaxGetElemLe context handlerVariable .one [handlerSlot] 0
      hlocals.2.1 hcontextHandlerOne (by simp)
    simpa using hbound
  have hreturnSlotNeHandler : returnSlot ≠ handlerSlot := by
    dsimp [returnSlot]
    omega
  have hglobalPayload :
      updateCrepRuntimeGlobal targetState.globals (0 : BitVec 5)
        (.word value) (0 : BitVec 5) = some (.word value) := by
    simp [updateCrepRuntimeGlobal]
  have hreturnSlotDistinct : [returnSlot].eraseDups.length = 1 := by
    simp [List.eraseDups, List.eraseDupsBy, List.eraseDupsBy.loop]
  have hcompiledCall : compileCodeRelProg context program = targetProgram := by
    simp [program, handlerProgram, targetProgram, returnSlot,
      handlerContinuation, compileCodeRelProg, compileProgHOL,
      compileExpHOL, compileArgsHOL, functionReturnNamesHOL, allocatedNamesHOL,
      hfunctionContext, hcontextExceptionLookup,
      hcontextHandlerOne, expHdlFiniteMap, loadGlobals, panMap2,
      crepNestedSeq, nestedDecs]
  have htargetRun : evalCrepRuntimeResult targetHandler targetPrimitive 12
      targetState (compileCodeRelProg context program) =
        some (.returned [value], targetPost) := by
    rw [hcompiledCall]
    simp [targetProgram, targetBody, handlerContinuation, targetPost,
      evalCrepRuntimeResult, evalCrepRuntimeProg, evalCrepRuntimeCall,
      evalCrepRuntimeExps, evalCrepRuntimeExp, htargetCallLookup,
      htargetClock, crepRuntimeCallInfoValid, decCrepClock,
      fixCrepRuntimeClock, crepRuntimeCallerState,
      restoreCrepRuntimeStep, clearCrepRuntimeLocals,
      updateCrepRuntimeLocal, updateCrepRuntimeGlobal,
      setCrepRuntimeGlobals, panTheWord,
      htargetHandlerLocal, hreturnSlotNeHandler,
      hreturnSlotDistinct, beq_iff_eq]
    funext candidate
