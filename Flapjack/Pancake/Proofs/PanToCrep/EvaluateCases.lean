...
/-! A concrete recursive continuation case for `DecCall`.  The continuation
is a clocked `While` constructor (with a false condition), so the IH is used
at the state produced by the state-owned code-map call.  In particular, the
IH returns the target continuation execution and all four post-relations; no
target run or post-state relation is a premise.  This is induction support,
not a standalone HOL theorem or a claim that the enclosing `pc_compile_correct`
...
theorem panToCrepPcCompileCorrectDecCallWhileFalseCodeStateRiscV64IH
    ...
    (hsource : panSemEvaluateRiscV64CodeState sourceContext sourcePrimitive
      ...
      some (.control (.normal sourceState.locals sourceState.globals
        sourceState.memory sourceState.ffi), decPanClock sourceState.clock))
    (hcontinuationSourceRun :
      evalPanValueFfiClockCodeProg sourceContext sourcePrimitive sourceHandler
        sourceState.structs sourceState.code sourceState.exceptionShapes
        sourceState.baseAddress sourceState.topAddress panSemBitVec64BytesInWord
        ...
        sourceState.globals sourceState.memory sourceState.ffi
        (decPanClock sourceState.clock) (.while (.const 0) .skip)
  ...
  have hsourceBodyRun :
    ...
    simpa [sourceBodyState, continuationResult] using hcontinuationSourceRun
