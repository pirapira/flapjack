...
/-! A concrete recursive continuation case for `DecCall`. The false `While`
continuation run is reduced from the production evaluator itself, then the
recursive source-run IH derives the target continuation execution and all four
post-relations. No target run or post-state relation is a premise. This is
induction support, not a standalone HOL theorem or a claim that the enclosing
`pc_compile_correct` induction is complete. -/
theorem panToCrepPcCompileCorrectDecCallWhileFalseCodeStateRiscV64IH
    (context : PanToCrepProofContext (RiscV.Word 64))
    (sourceContext : PanValueFfiContext (RiscV.Word 64))
    (sourcePrimitive : PanPrimitiveHandler (RiscV.Word 64))
    (sourceHandler : PanValueStatefulFfiHandler (RiscV.Word 64) σ)
    ...
    (hentry : panSemCodeLookup sourceState.code function =
      some ([ ("parameter", .one) ],
        .return (.var .local "parameter"), .one))
    (hclock : sourceState.clock ≠ 0)
    (hsource : panSemEvaluateRiscV64CodeState sourceContext sourcePrimitive
      sourceHandler sourceState
      (.decCall name .one function [.const value]
        (.while (.const 0) .skip) : Prog (RiscV.Word 64)) =
      some (.control (.normal sourceState.locals sourceState.globals
        sourceState.memory sourceState.ffi), decPanClock sourceState.clock))
    (hcontinuationIH :
      let targetSlot := context.vmax + 1
      let nextContext := { context with
        vars := FUPDATE context.vars
          (name, (Shape.one, [targetSlot]))
        vmax := context.vmax + 1 }
      let sourceBodyState : PanSemState (RiscV.Word 64) (FfiState σ) :=
        { sourceState with
          locals := updatePanValueMap sourceState.locals name (.word value)
          clock := decPanClock sourceState.clock }
      let targetBodyState : CrepRuntimeState (RiscV.Word 64) σ :=
        { targetState with
          locals := updateCrepRuntimeLocal targetState.locals targetSlot
            (.word value)
          clock := targetState.clock - 1 }
      ∀ bodyResult,
        evalPanValueFfiClockCodeProg sourceContext sourcePrimitive sourceHandler
          sourceBodyState.structs sourceBodyState.code
          sourceBodyState.exceptionShapes sourceBodyState.baseAddress
          sourceBodyState.topAddress panSemBitVec64BytesInWord
          (panSemCodeEvaluateFuel sourceState
            (.decCall name .one function [.const value]
              (.while (.const 0) .skip : Prog (RiscV.Word 64))) - 1)
          sourceBodyState.locals sourceBodyState.globals sourceBodyState.memory
          sourceBodyState.ffi sourceBodyState.clock
          (.while (.const 0) .skip)
          (memoryAccess := some (panSemBitVec64MemoryAccess sourceState)) =
            some bodyResult →
        stateRel sourceBodyState targetBodyState →
        codeRel nextContext (panSemCodeAsLookup sourceBodyState.code)
          targetBodyState.code →
        excpRel nextContext.eids sourceBodyState.exceptionShapes →
        localsRel nextContext sourceBodyState.locals targetBodyState.locals →
        ∃ targetBodyPost,
          evalCrepRuntimeProg targetHandler targetPrimitive 6 targetBodyState
            (.while (.const 0) .skip) = some (.normal, targetBodyPost) ∧
          stateRel (panSemCodeStateAfter sourceBodyState bodyResult)
            targetBodyPost ∧
          codeRel context
            (panSemCodeAsLookup
              (panSemCodeStateAfter sourceBodyState bodyResult).code)
            targetBodyPost.code ∧
          excpRel context.eids
            (panSemCodeStateAfter sourceBodyState bodyResult).exceptionShapes ∧
          localsRel nextContext
            (panSemCodeStateAfter sourceBodyState bodyResult).locals
            targetBodyPost.locals) :
    let program : Prog (RiscV.Word 64) :=
      .decCall name .one function [.const value]
        (.while (.const 0) .skip)
    let sourceResult : PanValueFfiClockResult (RiscV.Word 64) σ :=
    ...
    ∃ targetPost,
      ...
      excpRel context.eids
        (panSemCodeStateAfter sourceState sourceResult).exceptionShapes ∧
      localsRel context
        (panSemCodeStateAfter sourceState sourceResult).locals targetPost.locals := by
  let targetSlot := context.vmax + 1
  let nextContext : PanToCrepProofContext (RiscV.Word 64) :=
    { context with
      vars := FUPDATE context.vars (name, (Shape.one, [targetSlot]))
      vmax := context.vmax + 1 }
  let program : Prog (RiscV.Word 64) :=
    .decCall name .one function [.const value]
      (.while (.const 0) .skip)
  ...
  let continuationResult : PanValueFfiClockResult (RiscV.Word 64) σ :=
    (.control (.normal
      ...
      sourceState.globals sourceState.memory sourceState.ffi),
      decPanClock sourceState.clock)
  have hbodyRel : stateRel sourceBodyState targetBodyState := by
    rcases hstate with ⟨hmem, hmemaddrs, hshared, hstructs, hglobals,
      hclockRel, hbe, hffi, hbase, htop⟩
    simp [sourceBodyState, targetBodyState, stateRel, decPanClock,
      hmem, hmemaddrs, hshared, hstructs, hglobals,
      hclockRel, hbe, hffi, hbase, htop]
  have hbodyCode : codeRel nextContext
      (panSemCodeAsLookup sourceBodyState.code) targetBodyState.code := by
    change codeRel context (panSemCodeAsLookup sourceState.code) targetState.code
    exact hcode
  have hbodyExcp : excpRel nextContext.eids sourceBodyState.exceptionShapes := by
    change excpRel context.eids sourceState.exceptionShapes
    exact hexcp
  have hbodyLocals : localsRel nextContext sourceBodyState.locals
      targetBodyState.locals := by
    have hslotFresh : targetSlot = context.vmax + 1 := rfl
    have hshape : panValueShape [] (.word value) = Shape.one := by
      simp [panValueShape]
    have hshapeWf : isWfShape [] (panValueShape [] (.word value)) = true := by
      rw [hshape]
      simp [isWfShape]
    have hlength : [targetSlot].length =
        Shape.shapeSize (panValueShape [] (.word value)) := by
      simp [hshape]
    have hbounds : ∀ slot, slot ∈ [targetSlot] →
        context.vmax < slot ∧
          slot ≤ context.vmax + Shape.shapeSize (panValueShape [] (.word value)) := by
      intro slot hslot
      simp only [List.mem_singleton] at hslot
      subst slot
      rw [hshape]
      simp [targetSlot]
    have hrel := localsRelExtendNewVar context sourceState.locals
      targetState.locals (.word value) name [targetSlot] hlocals hshapeWf
      (by simp) hbounds hlength
    have hsourceUpdate : updatePanValueMap sourceState.locals name
        (.word value) = FUPDATE sourceState.locals (name, .word value) := by
      funext key
      by_cases hkey : key = name
      · subst key
        simp [updatePanValueMap, FUPDATE]
      · have hkn : (key == name) = false := beq_eq_false_iff_ne.mpr hkey
        have hnk : (name == key) = false :=
          beq_eq_false_iff_ne.mpr (Ne.symm hkey)
        simp [updatePanValueMap, FUPDATE, hkn, hnk]
    change localsRel nextContext
      (updatePanValueMap sourceState.locals name (.word value))
      (updateCrepRuntimeLocal targetState.locals targetSlot (.word value))
    rw [hsourceUpdate, updateCrepRuntimeLocal_eq_FUPDATE]
    simpa [nextContext, sourceBodyState, targetBodyState,
      FUPDATE_LIST, targetSlot, hshape, panValueShape, panValueFlatten,
      Shape.shapeSize] using hrel
  have hsourceBodyRun :
      evalPanValueFfiClockCodeProg sourceContext sourcePrimitive sourceHandler
        sourceBodyState.structs sourceBodyState.code
        sourceBodyState.exceptionShapes sourceBodyState.baseAddress
        sourceBodyState.topAddress panSemBitVec64BytesInWord
        (panSemCodeEvaluateFuel sourceState program - 1)
        sourceBodyState.locals sourceBodyState.globals sourceBodyState.memory
        sourceBodyState.ffi sourceBodyState.clock (.while (.const 0) .skip)
        (memoryAccess := some (panSemBitVec64MemoryAccess sourceState)) =
          some continuationResult := by
    have hfuelBound : 2 ≤ panSemCodeEvaluateFuel sourceState program := by
      have hprogramFuel := panSemProgFuel_pos program
      have hclockFactor : 0 < sourceState.clock + 1 := by omega
      have hcostBound : 1 ≤ max (panSemProgFuel program)
          (panSemCodeBodyFuel sourceState.code) :=
        Nat.le_trans hprogramFuel (Nat.le_max_left _ _)
      have hcostFactor : 0 < max (panSemProgFuel program)
          (panSemCodeBodyFuel sourceState.code) + 1 := by omega
      have hproduct := Nat.mul_pos hclockFactor hcostFactor
      unfold panSemCodeEvaluateFuel
      omega
    have hfuelPositive : 0 < panSemCodeEvaluateFuel sourceState program - 1 := by
      omega
    cases hfuel : panSemCodeEvaluateFuel sourceState program - 1 with
    | zero => omega
    | succ fuel =>
        simp [evalPanValueFfiClockCodeProg, sourceBodyState,
          continuationResult, panValueIteConditionValue, evalPanValueExp]
  have hbodyIH := hcontinuationIH continuationResult hsourceBodyRun
    hbodyRel hbodyCode hbodyExcp hbodyLocals
  rcases hbodyIH with ⟨targetBodyPost, htargetBody, hbodyPostState,
    hbodyPostCode, hbodyPostExcp, hbodyPostLocals⟩
  have htargetBodyPost : targetBodyPost = targetBodyState := by
    have hrun := htargetBody
    simp [evalCrepRuntimeProg, evalCrepRuntimeExp] at hrun
    exact hrun.symm
  subst targetBodyPost
  have htargetCodeLookup : FLOOKUP targetState.code function =
      some ([0], .return [.var 0]) := by
    have hsourceLookup : FLOOKUP (panSemCodeAsLookup sourceState.code)
        function = some ([ ("parameter", .one) ],
          .return (.var .local "parameter"), .one) := by
      change panSemCodeLookup sourceState.code function = _
      exact hentry
    rcases codeRelImp context (panSemCodeAsLookup sourceState.code)
        targetState.code hcode function [ ("parameter", .one) ]
        (.return (.var .local "parameter")) .one hsourceLookup with
      ⟨_, _, htargetLookup⟩
    simpa [Shape.shapeSize, ctxtFc, withShape, FUPDATE_LIST, FUPDATE,
      FLOOKUP_update, compileCodeRelProg, compileProgHOL, compileExpHOL] using
        htargetLookup
  have htargetCallLookup : lookupCrepRuntimeCode function [value]
      targetState.code = some (.return [.var 0], fun key =>
        if key == 0 then some (.word value) else none) := by
    simp [lookupCrepRuntimeCode, lookupCrepHolCode, htargetCodeLookup,
      FUPDATE_LIST]
    funext key
    ...
    · have hzero : (0 == key) = false := by simp [Ne.symm hkey]
      simp [FUPDATE, FEMPTY, hzero, hkey]
  have hclockRel : sourceState.clock = targetState.clock := by
    rcases hstate with ⟨_, _, _, _, _, hclockRel, _, _, _, _⟩
    exact hclockRel
  have htargetClock : targetState.clock ≠ 0 := by
    rw [← hclockRel]
    exact hclock
  have hcompiled : compileCodeRelProg context program =
      .dec targetSlot (.const 0)
        (.seq (.call (some ([targetSlot], none)) function [.const value])
          (.while (.const 0) .skip)) := by
    simp [program, targetSlot, compileCodeRelProg,
      compileProgHOL, compileArgsHOL, compileExpHOL, allocatedNamesHOL,
      nestedDecs]
  have hcallNames : ([targetSlot].eraseDups).length = 1 := by
    simp [List.eraseDups, List.eraseDupsBy, List.eraseDupsBy.loop]
  have hcallInfo : crepRuntimeCallInfoValid
      (some ([targetSlot], none) : Option (List Nat ×
        Option ((RiscV.Word 64) × CrepProg (RiscV.Word 64)))) = true := by
    simpa [crepRuntimeCallInfoValid] using hcallNames
  let caller0 : CrepRuntimeState (RiscV.Word 64) σ :=
    { targetState with
      locals := updateCrepRuntimeLocal targetState.locals targetSlot (.word 0) }
  let calleeLocals : Nat → Option (PanWordLab (RiscV.Word 64)) :=
    fun key => if key == 0 then some (.word value) else none
  let targetCallee := decCrepClock { caller0 with locals := calleeLocals }
  let targetCalleePost := clearCrepRuntimeLocals targetCallee
  have htargetCalleeBody : evalCrepRuntimeProg targetHandler targetPrimitive 4
      targetCallee (.return [.var 0]) =
        some (.returned [value], targetCalleePost) := by
    rw [evalCrepRuntimeProg]
    simp [targetCallee, targetCalleePost, calleeLocals,
      decCrepClock, evalCrepRuntimeExp, evalCrepRuntimeExps,
      panTheWord, clearCrepRuntimeLocals]
  have hlocalsUpdate :
      updateCrepRuntimeLocal
        (updateCrepRuntimeLocal targetState.locals targetSlot (.word 0))
        targetSlot (.word value) =
      updateCrepRuntimeLocal targetState.locals targetSlot (.word value) := by
    funext key
    by_cases hkey : targetSlot = key
    · subst key
      simp [updateCrepRuntimeLocal]
    · simp [updateCrepRuntimeLocal, hkey]
  have hlookup : lookupCrepRuntimeCode function [value] caller0.code =
      some (.return [.var 0], calleeLocals) := by
    change lookupCrepRuntimeCode function [value] targetState.code = _
    rw [htargetCallLookup]
  have hcallerClock : caller0.clock ≠ 0 := by
    simpa [caller0] using htargetClock
  have htargetCall : evalCrepRuntimeProg targetHandler targetPrimitive 6
      caller0
      (.call (some ([targetSlot], none)) function [.const value]) =
      some (.normal, targetBodyState) := by
    rw [evalCrepRuntimeProg]
    unfold evalCrepRuntimeCall
    simp [evalCrepRuntimeExps, evalCrepRuntimeExp]
    rw [hlookup]
    simp only [hcallInfo]
    simp only [if_neg hcallerClock]
    rw [htargetCalleeBody]
    simp [fixCrepRuntimeClock, crepRuntimeCallerState, caller0,
      targetCalleePost, targetCallee, clearCrepRuntimeLocals,
      targetBodyState, targetSlot, crepRuntimeAssignExisting,
      hcallNames, updateCrepRuntimeLocal, beq_iff_eq, decCrepClock]
    simpa [targetSlot] using hlocalsUpdate
  have hfixCall : fixCrepRuntimeClock caller0
      ((.normal : CrepRuntimeResult (RiscV.Word 64) FfiFinalEvent),
        targetBodyState) =
      ((.normal : CrepRuntimeResult (RiscV.Word 64) FfiFinalEvent),
        targetBodyState) := by
    simp [fixCrepRuntimeClock, caller0, targetBodyState]
  have hseq : evalCrepRuntimeProg targetHandler targetPrimitive 7
      caller0
      (.seq (.call (some ([targetSlot], none)) function [.const value])
        (.while (.const 0) .skip)) = some (.normal, targetBodyState) := by
    rw [evalCrepRuntimeProg]
    simp only [htargetCall, hfixCall]
    rw [htargetBody]
  have hrestoreLocals :
      resVar (updateCrepRuntimeLocal targetState.locals targetSlot (.word value))
        (targetSlot, targetState.locals targetSlot) = targetState.locals := by
    funext key
    by_cases hkey : key = targetSlot
    · subst key
      simp [resVar_locals_apply]
    · simp [resVar_locals_apply, updateCrepRuntimeLocal, Ne.symm hkey]
  have htargetRun : evalCrepRuntimeResult targetHandler targetPrimitive 8
      targetState (compileCodeRelProg context program) =
        some (.normal, decCrepClock targetState) := by
    rw [hcompiled]
    simp only [evalCrepRuntimeResult]
    rw [evalCrepRuntimeProg]
    simp only [evalCrepRuntimeExp, setCrepRuntimeLocal_eq_update]
    rw [hseq]
    simp only [restoreCrepRuntimeStep_eq_resVar]
    apply congrArg some
    apply Prod.ext
    · rfl
    · cases targetState
      simp [targetBodyState, targetSlot, resVar]
      simpa [targetSlot, resVar_locals_apply,
        resVar, updateCrepRuntimeLocal, FUPDATE, FDOMSUB, beq_iff_eq,
        decCrepClock] using hrestoreLocals
  refine ⟨hsource, decCrepClock targetState, htargetRun, ?_, ?_, ?_, ?_⟩
  · rcases hstate with ⟨hmem, hmemaddrs, hshared, hstructs, hglobals,
      hclockRel', hbe, hffi, hbase, htop⟩
    simp [panSemCodeStateAfter, stateRel, decPanClock,
      decCrepClock, hmem, hmemaddrs, hshared, hstructs, hglobals,
      hclockRel', hbe, hffi, hbase, htop]
  · change codeRel context (panSemCodeAsLookup sourceState.code) targetState.code
    exact hcode
  · change excpRel context.eids sourceState.exceptionShapes
    exact hexcp
  · change localsRel context sourceState.locals targetState.locals
    exact hlocals
