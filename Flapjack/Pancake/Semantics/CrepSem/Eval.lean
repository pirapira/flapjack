...
/-! ## Total clocked crepSem evaluator over `CrepHolState`

`evalCrepHolProgTotal` is a genuine total evaluator over the 11-field
`CrepHolState`: it returns a pair `(result, state)` directly (no `Option`, no
fuel), mirroring the total signature of HOL `crepSem$evaluate_def`
(`cakeml/pancake/semantics/crepSemScript.sml:240`).  This first commit-sized
slice implements the exact `Skip`/`Break`/`Continue` clauses plus the
purely-structural and local-state clauses (`Tick`, `Raise`, `Return`,
`Assign`, `StoreGlob`, `Seq`, `If`, `Dec`), which is where HOL's clock/error
behaviour is visible without an FFI oracle.

Declaration-local mismatch notes (this slice is untagged):

* `While`, `Call`, `ExtCall`, `ShMem`, `Primitive`, `Store`, `Store32` and
  `StoreByte` are not yet faithful: they currently return
  `(CrepRuntimeResult.error, state)`.  Modelling them needs the loop/clock
  recursion and the target memory/FFI handlers, tracked as follow-up children
  of `flapjack-pxn.18.5.5.7.7`.
* The evaluator is `[NeZero width]` over `RiscV.Word width` rather than HOL's
  arbitrary word carrier.
* `Dec` restores the shadowed local unconditionally after the body, a
  simplification of HOL's `res_var` bookkeeping; `Seq`/`Dec` do not yet apply
  HOL's `fix_clock` clamp.

Only `Skip`/`Break`/`Continue` are proven exact below; the remaining clauses
are provisional and carry no HOL tag. -/
def evalCrepHolProgTotal [NeZero width]
    (handler : CrepRuntimeFfiHandler (RiscV.Word width) σ ε)
    (primitive : CrepPrimitiveHandler (RiscV.Word width)) :
    CrepProg (RiscV.Word width) → CrepHolState (RiscV.Word width) σ →
      CrepRuntimeResult (RiscV.Word width) ε × CrepHolState (RiscV.Word width) σ
  | .skip, state => (CrepRuntimeResult.normal, state)
  | .break label, state => (CrepRuntimeResult.broke label, state)
  | .continue label, state => (CrepRuntimeResult.continued label, state)
  | .tick, state =>
      if state.clock = 0
      then (CrepRuntimeResult.timeout, { state with locals := fun _ => none })
      else (CrepRuntimeResult.normal, { state with clock := state.clock - 1 })
  | .raise exception, state =>
      (CrepRuntimeResult.raised exception, { state with locals := fun _ => none })
  | .return values, state =>
      match values.mapM (evalCrepHolExp state) with
      | some result =>
          (CrepRuntimeResult.returned result, { state with locals := fun _ => none })
      | none => (CrepRuntimeResult.error, state)
  | .assign name value, state =>
      match evalCrepHolExp state value with
      | none => (CrepRuntimeResult.error, state)
      | some value =>
          ...
          | some _ =>
              (CrepRuntimeResult.normal,
                { state with
                  locals := updateCrepRuntimeLocal state.locals name (.word value) })
          | none => (CrepRuntimeResult.error, state)
  | .storeGlob address value, state =>
      match evalCrepHolExp state value with
      | some value =>
          (CrepRuntimeResult.normal, setCrepHolGlobals address (.word value) state)
      | none => (CrepRuntimeResult.error, state)
  | .seq first second, state =>
      match evalCrepHolProgTotal handler primitive first state with
      | (CrepRuntimeResult.normal, state') =>
          evalCrepHolProgTotal handler primitive second state'
      | result => result
  | .ite condition thenBranch elseBranch, state =>
      ...
      | some conditionValue =>
          if conditionValue != 0
          then evalCrepHolProgTotal handler primitive thenBranch state
          else evalCrepHolProgTotal handler primitive elseBranch state
  | .dec name value body, state =>
      match evalCrepHolExp state value with
      | none => (CrepRuntimeResult.error, state)
      | some value =>
          let previous := state.locals name
          let nextState :=
            { state with locals := updateCrepRuntimeLocal state.locals name (.word value) }
          match evalCrepHolProgTotal handler primitive body nextState with
          | (result, state') =>
              (result,
                { state' with
                  locals := fun n => if n == name then previous else state'.locals n })
  | _, state => (CrepRuntimeResult.error, state)
termination_by program _ => sizeOf program
decreasing_by all_goals decreasing_trivial

/-- Total `Skip` clause of `evalCrepHolProgTotal`, matching HOL
    `evaluate (Skip, s) = (NONE, s)`. -/
theorem evalCrepHolProgTotal_skip [NeZero width]
    (handler : CrepRuntimeFfiHandler (RiscV.Word width) σ ε)
    (primitive : CrepPrimitiveHandler (RiscV.Word width))
    (state : CrepHolState (RiscV.Word width) σ) :
    evalCrepHolProgTotal handler primitive .skip state =
      (CrepRuntimeResult.normal, state) := by
  simp only [evalCrepHolProgTotal]

/-- Total `Break` clause of `evalCrepHolProgTotal`, matching HOL
    `evaluate (Break n, s) = (SOME (Break n), s)`. -/
theorem evalCrepHolProgTotal_break [NeZero width]
    (handler : CrepRuntimeFfiHandler (RiscV.Word width) σ ε)
    (primitive : CrepPrimitiveHandler (RiscV.Word width))
    (state : CrepHolState (RiscV.Word width) σ) (label : Nat) :
    evalCrepHolProgTotal handler primitive (.break label) state =
      (CrepRuntimeResult.broke label, state) := by
  simp only [evalCrepHolProgTotal]

/-- Total `Continue` clause of `evalCrepHolProgTotal`, matching HOL
    `evaluate (Continue n, s) = (SOME (Continue n), s)`. -/
theorem evalCrepHolProgTotal_continue [NeZero width]
    (handler : CrepRuntimeFfiHandler (RiscV.Word width) σ ε)
    (primitive : CrepPrimitiveHandler (RiscV.Word width))
    (state : CrepHolState (RiscV.Word width) σ) (label : Nat) :
    evalCrepHolProgTotal handler primitive (.continue label) state =
      (CrepRuntimeResult.continued label, state) := by
  simp only [evalCrepHolProgTotal]
