...
/-- HOL `panProps$evaluate_decls_functions` (`panPropsScript.sml:1518-1526`):
    ...
    in `code`. The result state is quantified explicitly and related by
    `= some result`, the repository's rendering of HOL's `SOME s'` premise;
    `updateList` is the canonical finite-map `|++`. The
    `fmap_as_finite_support` qualifier records only the four `|->` fields. -/
@[hol "cakeml/pancake/semantics/panPropsScript.sml" "evaluate_decls_functions"
  (fmap_as_finite_support := [locals, globals, code, eshapes])]
theorem evaluateDeclsPanPropsHOLFinite_functions {width : Nat} {σ : Type} [NeZero width] :
    ∀ (state : PanPropsEvalStateFiniteExact width σ) [DecidablePred state.memaddrs]
  ...
  | nil =>
      ...
      simp only [evaluateDeclsPanPropsHOLFinite, Option.some.injEq] at hresult
      subst hresult
  ...
  | cons declaration rest ih =>
      ...
      cases declaration with
      | name name fields =>
          simp only [evaluateDeclsPanPropsHOLFinite, functionsHOL] at hresult ⊢
