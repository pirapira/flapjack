...
def wordInstReadVars {α : Type u} : WordInst α → List Nat
  ...
  | .const _ _ => []
  | .binop _ _ source right =>
      match right with
      | .reg name => [source, name]
      | .imm _ => [source]
  | .shiftInst _ _ source amount =>
      match amount with
      | .reg name => [source, name]
      | .imm _ => [source]
  | .memOffset operator destination base _ =>
      match operator with
      | .load | .load8 | .load16 | .load32 => [base]
      | .store | .store8 | .store16 | .store32 => [destination, base]
...
def wordInstWriteVars {α : Type u} : WordInst α → List Nat
  ...
  | .shiftInst _ destination _ _ => [destination]
...
def wordLinearClashAnalysis {α : Type u} : List (WordInst α) → List Nat →
...
def wordSsaRenameInst {α : Type u} (state : WordSsaState) : WordInst α → WordSsaState × WordInst α
  | .arith operation =>
      match operation with
  ...
  | .binop operator destination source right =>
      ...
      let right := match right with
        ...
        | .imm value => .imm value
      let (state, freshDestination) := wordSsaFresh state destination
      (state, .binop operator freshDestination source right)
  | .shiftInst operator destination source amount =>
      ...
      let amount := match amount with
        ...
        | .imm value => .imm value
      let (state, freshDestination) := wordSsaFresh state destination
      (state, .shiftInst operator freshDestination source amount)
  | .memOffset operator destination base offset =>
      let base := wordSsaRead state base
      match operator with
      | .load | .load8 | .load16 | .load32 =>
          let (state, freshDestination) := wordSsaFresh state destination
          (state, .memOffset operator freshDestination base offset)
      | .store | .store8 | .store16 | .store32 =>
          (state, .memOffset operator (wordSsaRead state destination) base offset)

def wordSsaRenameLinear {α : Type u} (state : WordSsaState) : List (WordInst α) →
...
def wordAllocateSsaLinear {α : Type u} (state : WordSsaState) (instructions : List (WordInst α)) :
...
def wordClashTreeDeltaInst {α : Type u} : WordInst α → WordClashTree
  ...
  | .binop _ destination _ _ => .delta [destination] []
  | .shiftInst _ destination _ _ => .delta [destination] []
  | .memOffset operator destination base _ =>
      match operator with
      | .load | .load8 | .load16 | .load32 => .delta [destination] [base]
      | .store | .store8 | .store16 | .store32 => .delta [] [destination, base]
...
def wordApplyColourInst {α : Type u} (colour : Nat → Nat) : WordInst α → WordInst α
  ...
  | .binop operator destination source right =>
      .binop operator (colour destination) (colour source)
        (wordApplyColourRegImm colour right)
  | .shiftInst operator destination source amount =>
      .shiftInst operator (colour destination) (colour source)
        (wordApplyColourRegImm colour amount)
  | .memOffset operator destination base offset =>
      .memOffset operator (colour destination) (colour base) offset
...
def wordProgSpecialLocationsSafe (locations : NatInfoMap WordLocation) :
  ...
  | .inst (.mem _ _ _) => true
  | .inst (.const _ _) | .inst (.binop _ _ _ _) | .inst (.shiftInst _ _ _ _) |
      .inst (.memOffset _ _ _ _) => true
  | .seq first second =>
...
/-- Backend operand-distinctness conflicts that the CakeML-shaped clash tree
    does not record.  The RISC-V `LongMul` selection writes the high word
    before the low word, so the high destination must differ from both
...
def wordProgSpecialConflictEdges : WordProg α → List (Nat × Nat)
  | .inst (.arith operation) => wordArithSpecialConflictEdges operation
  | .seq first second =>
      wordProgSpecialConflictEdges first ++ wordProgSpecialConflictEdges second
  | .ite _ _ _ thenBranch elseBranch =>
      wordProgSpecialConflictEdges thenBranch ++ wordProgSpecialConflictEdges elseBranch
  | .loop _ body _ => wordProgSpecialConflictEdges body
  | .mustTerminate body => wordProgSpecialConflictEdges body
  | .call (some (_, _, returnCode, _, _)) _ _ none =>
      wordProgSpecialConflictEdges returnCode
  | .call (some (_, _, returnCode, _, _)) _ _ (some (_, body, _, _)) =>
      wordProgSpecialConflictEdges body ++ wordProgSpecialConflictEdges returnCode
  | _ => []
...
theorem wordGreedyAllocateWithSpillsAndPreferencesClashIndexed_preserves_lookup
    (names : List Nat) (edges preferences : List (Nat × Nat))
    (clashIndex preferenceIndex : Std.HashMap Nat (List Nat))
    (locations : Std.HashMap Nat WordLocation) (state : WordSpillState) :
    ∀ name, name ∉ names →
      lookupNatInfo name
          (wordGreedyAllocateWithSpillsAndPreferencesClashIndexed names edges preferences
            clashIndex preferenceIndex locations state).locations =
        lookupNatInfo name state.locations := by
  induction names generalizing locations state with
  | nil =>
      intro name hname
  ...
  | cons head tail ih =>
      ...
      let forbidden :=
        wordUsedLocationRegistersIndexed
          (wordClashNeighboursIndexed clashIndex head) locations
      let candidates := wordColourCandidatesWithSpillPreferencesIndexed head
        preferences preferenceIndex locations
      let available := wordFirstAvailable candidates forbidden
      let allocated := match available with
      ...
      | none =>
          calc
            lookupNatInfo name
                (wordGreedyAllocateWithSpillsAndPreferencesClashIndexed (head :: tail)
                  edges preferences clashIndex preferenceIndex locations state).locations =
                lookupNatInfo name
                  (wordGreedyAllocateWithSpillsAndPreferencesClashIndexed tail edges preferences
                    clashIndex preferenceIndex locations' allocated).locations := by
                      simp [wordGreedyAllocateWithSpillsAndPreferencesClashIndexed,
                        forbidden, candidates, available, allocated, locations', havailable]
            _ = lookupNatInfo name allocated.locations := htail
            _ = lookupNatInfo name state.locations := by
              simp [allocated, havailable, lookupNatInfo, hneq']
...
theorem wordGreedyAllocateWithSpillsAndPreferencesClashIndexed_maps_names
    (names : List Nat) (edges preferences : List (Nat × Nat))
    (clashIndex preferenceIndex : Std.HashMap Nat (List Nat))
    (locations : Std.HashMap Nat WordLocation) (state : WordSpillState) :
    ∀ name, name ∈ names →
      ∃ location, lookupNatInfo name
          (wordGreedyAllocateWithSpillsAndPreferencesClashIndexed names edges preferences
            clashIndex preferenceIndex locations state).locations = some location := by
  induction names generalizing locations state with
  | nil =>
      intro name hname
      simp at hname
  | cons head tail ih =>
      ...
      | none =>
          ...
          · subst name
            by_cases hdup : head ∈ tail
            · simpa [wordGreedyAllocateWithSpillsAndPreferencesClashIndexed,
                forbidden, candidates, available, allocated, locations', havailable] using
                ih locations' allocated head hdup
            · have htail_lookup :=
                wordGreedyAllocateWithSpillsAndPreferencesClashIndexed_preserves_lookup
                  tail edges preferences clashIndex preferenceIndex locations' allocated head hdup
              refine ⟨.stack state.nextSpill, ?_⟩
              calc
                lookupNatInfo head
                    (wordGreedyAllocateWithSpillsAndPreferencesClashIndexed (head :: tail)
...
/-- Respecting a superset of interference edges implies respecting any prefix
    of them.  Used when a caller adds backend-specific conflict edges after the
    CakeML-shaped clash-tree edges. -/
theorem wordSpillAllocationRespectsClashes_append_left
    (edges extra : List (Nat × Nat)) (locations : NatInfoMap WordLocation)
    (h : wordSpillAllocationRespectsClashes (edges ++ extra) locations = true) :
    wordSpillAllocationRespectsClashes edges locations = true := by
  induction edges with
  | nil => rfl
  | cons head tail ih =>
      obtain ⟨left, right⟩ := head
      simp only [List.cons_append, wordSpillAllocationRespectsClashes] at h ⊢
      split at h <;> simp_all
...
theorem wordAllocateVarsWithFixedSourcesClashFast_preserves_fixed_source
    (slots : List Nat) (edges preferences : List (Nat × Nat))
    (fixedSources : List Nat) (state : WordSpillState)
    (hstate : wordAllocateVarsWithFixedSourcesClashFast slots edges preferences
      fixedSources = some state) (name : Nat) (hname : name ∈ fixedSources) :
    lookupNatInfo name state.locations = some (.register name) := by
  let initial : WordSpillState :=
    { locations := wordFixedSourceLocations fixedSources, nextSpill := 0 }
  let names := slots.eraseDups.filter (fun candidate => candidate ∉ fixedSources)
  let allocated := wordGreedyAllocateWithSpillsAndPreferencesClashFast names
    edges preferences initial
  have hstate' :
      (if wordSpillAllocationRespectsClashesFast edges allocated.locations = true then
          some allocated else none) = some state := by
    simpa [wordAllocateVarsWithFixedSourcesClashFast,
      wordAllocateVarsWithFixedLocationsClashIndexed, initial, names, allocated] using hstate
  split at hstate'
  · have heq : allocated = state := Option.some.inj hstate'
    subst state
    have hnot : name ∉ names := by
    ...
    have hlookup :=
      wordGreedyAllocateWithSpillsAndPreferencesClashIndexed_preserves_lookup
        names edges preferences (wordClashIndex edges) (wordPreferenceIndex preferences)
        (wordLocationIndex initial.locations) initial name hnot
    calc
      lookupNatInfo name allocated.locations =
          lookupNatInfo name initial.locations := by
        simpa [wordGreedyAllocateWithSpillsAndPreferencesClashFast, allocated] using hlookup
      _ = some (.register name) := by
        exact lookupNatInfo_wordFixedSourceLocations_mem_fast fixedSources name hname
  · contradiction

theorem wordAllocateVarsWithFixedSourcesClashFast_maps_slots
    (slots : List Nat) (edges preferences : List (Nat × Nat))
    (fixedSources : List Nat) (state : WordSpillState)
    (hstate : wordAllocateVarsWithFixedSourcesClashFast slots edges preferences
      fixedSources = some state) :
  ...
  · have heq : allocated = state := Option.some.inj hstate'
    ...
    · have hname : name ∈ names := by
        simp [names, hslot, hfixed]
      have hnames :=
        wordGreedyAllocateWithSpillsAndPreferencesClashIndexed_maps_names
          names edges preferences (wordClashIndex edges) (wordPreferenceIndex preferences)
          (wordLocationIndex initial.locations) initial name hname
      simpa [wordGreedyAllocateWithSpillsAndPreferencesClashFast, allocated] using hnames
  · contradiction

theorem wordAllocateVarsWithFixedSourcesClashFast_sound
    (slots : List Nat) (edges preferences : List (Nat × Nat))
    (fixedSources : List Nat) (state : WordSpillState)
    (hstate : wordAllocateVarsWithFixedSourcesClashFast slots edges preferences
      fixedSources = some state) :
    wordSpillAllocationRespectsClashes edges state.locations = true := by
  simp [wordAllocateVarsWithFixedSourcesClashFast,
    wordAllocateVarsWithFixedLocationsClashIndexed] at hstate
  rcases hstate with ⟨hcheck, heq⟩
  rw [wordSpillAllocationRespectsClashesFast_eq] at hcheck
  simpa [heq] using hcheck
...
theorem wordGreedyAllocateWithSpillsAndPreferences_preserves_lookup
    (names : List Nat) (edges preferences : List (Nat × Nat))
    (state : WordSpillState) :
    ∀ name, name ∉ names →
      lookupNatInfo name
          (wordGreedyAllocateWithSpillsAndPreferences names edges preferences
            state).locations =
        lookupNatInfo name state.locations := by
  induction names generalizing state with
  | nil =>
  ...
  | cons head tail ih =>
      ...
      let forbidden :=
        wordUsedLocationRegisters (wordNeighbours head edges) state.locations
      let candidates := wordColourCandidatesWithSpillPreferences head
        preferences state.locations
      let allocated := match wordFirstAvailable candidates forbidden with
        | some register =>
            { state with locations := (head, .register register) :: state.locations }
        | none =>
            { locations := (head, .stack state.nextSpill) :: state.locations,
      ...
      have hneq' : ¬ head == name := by
        ...
        have heq' : head = name := by simpa using heq
        exact hneq heq'.symm
      cases havailable : wordFirstAvailable candidates forbidden with
      | none =>
          have htail := ih allocated name (by
            intro htail
            apply hname
          ...
          calc
            lookupNatInfo name
                (wordGreedyAllocateWithSpillsAndPreferences (head :: tail)
                  edges preferences state).locations =
                lookupNatInfo name
                  (wordGreedyAllocateWithSpillsAndPreferences tail edges
                    preferences allocated).locations := by
                      simp [wordGreedyAllocateWithSpillsAndPreferences,
...
theorem wordGreedyAllocateWithSpillsAndPreferences_maps_names
    ...
    ∀ name, name ∈ names →
      ∃ location,
        lookupNatInfo name
          (wordGreedyAllocateWithSpillsAndPreferences names edges preferences
            state).locations = some location := by
  induction names generalizing state with
  | nil =>
      intro name hname
      simp at hname
      ...
      | none =>
          ...
          · subst name
            ...
            · simpa [wordGreedyAllocateWithSpillsAndPreferences, forbidden,
                candidates, allocated, havailable] using ih allocated head hdup
            · have htail_lookup :=
                wordGreedyAllocateWithSpillsAndPreferences_preserves_lookup tail
                  edges preferences allocated head hdup
              have hself : head == head := by simp
              refine ⟨.stack state.nextSpill, ?_⟩
              calc
                lookupNatInfo head
...
theorem wordAllocateVarsWithSpillsAndPreferences_maps_slots (slots : List Nat)
    ...
    ∀ name, name ∈ slots.eraseDups →
      ∃ location, lookupNatInfo name state.locations = some location := by
  let allocated : WordSpillState :=
    wordGreedyAllocateWithSpillsAndPreferences slots.eraseDups edges preferences
      { locations := [], nextSpill := 0 }
  have hstate' :
      (if wordSpillAllocationRespectsClashes edges allocated.locations = true then
          some allocated else none) = some state := by
    simpa [wordAllocateVarsWithSpillsAndPreferences, allocated] using hstate
  split at hstate'
  · have heq : allocated = state := Option.some.inj hstate'
    subst state
    simpa [allocated] using
      (wordGreedyAllocateWithSpillsAndPreferences_maps_names slots.eraseDups
        edges preferences { locations := [], nextSpill := 0 })
  · contradiction
