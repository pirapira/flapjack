...
def cakeMovesToSp : List (Nat × (Nat × Nat)) →
  ...
  | (p, (x, y)) :: rest, table =>
      /- `moves_to_sp` recurses over the tail before inserting the current
         ...
         Upstream this table is an sptree (`reg_allocScript.sml:1330-1345`),
         so each insert is logarithmic; as an association list every one of a
         function's moves rescanned the whole partner table.  The node-indexed
         field gives the same table with the same partner lists. -/
      let table := cakeMovesToSp rest table
      let partnersY := (table.get y).getD []
      let table := table.set y ((p, x) :: partnersY)
      let partnersX := (table.get x).getD []
      table.set x ((p, y) :: partnersX)
