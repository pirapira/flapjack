...
/-! Binary assignments are a useful local case of HOL's `evaluate_apply_colour`:
    both operands are read from the pre-state, so the destination can share a
    colour with either operand. Only variables that remain live after the
    write need to avoid the destination colour. This is untagged support over
    Flapjack's finite RISC-V state; it does not establish HOL's full Word
    evaluator relation, source permutation, or `colouring_ok` derivation. -/
theorem wordColourStateRelationOn_executeBinary
    (colour : Nat → Nat) (valid : wordColourValid colour)
    (colourZero : colour 0 = 0)
    (source target : State width) [NeZero width]
    (liveAfter : List Nat)
    ...
    (hrelation : WordColourStateRelationOn colour
      (left :: right :: liveAfter) source target)
    (hcolourNonzero : name ≠ 0 → colour name ≠ 0)
    (hnoAlias : ∀ current, current ∈ liveAfter → current ≠ name →
      colour current ≠ colour name) :
    WordColourStateRelationOn colour liveAfter
  ...
  have hleftValue := hrelation.register left (by simp) hleft (valid left hleft)
  have hrightValue := hrelation.register right (by simp) hright (valid right hright)
  ...
  | add =>
      ...
      simpa [execute] using
        (wordColourStateRelationOn_writeRegister colour valid colourZero
...
theorem evalWordProg_assignBinaryVarVar_applyColour_live
    (colour : Nat → Nat) (valid : wordColourValid colour)
    (colourZero : colour 0 = 0)
    (source target : State width) [NeZero width]
    (liveAfter : List Nat)
    ...
    (hnoAlias : ∀ current, current ∈ liveAfter → current ≠ name →
      colour current ≠ colour name) :
    ∃ source' target',
      evalWordProg source (.assign name (.op operator [.var left, .var right])) =
        some source' ∧
      evalWordProg target
  ...
  cases operator with
  | add =>
      refine ⟨execute source (.add ⟨name, hname⟩ ⟨left, hleft⟩ ⟨right, hright⟩),
        execute target (.add ⟨colour name, valid name hname⟩
          ⟨colour left, valid left hleft⟩ ⟨colour right, valid right hright⟩),
        ?_, ?_, ?_⟩
      ...
      · simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          ...
          executeInstructions]
      · exact wordColourStateRelationOn_executeBinary colour valid colourZero
          source target liveAfter .add name left right hname hleft hright
          hrelation hcolourNonzero hnoAlias
  | sub =>
      refine ⟨execute source (.sub ⟨name, hname⟩ ⟨left, hleft⟩ ⟨right, hright⟩),
        execute target (.sub ⟨colour name, valid name hname⟩
          ⟨colour left, valid left hleft⟩ ⟨colour right, valid right hright⟩),
        ?_, ?_, ?_⟩
      · simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,
          registerOfNat, hname, hleft, hright, executeInstructions]
      · simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          ...
          executeInstructions]
      · exact wordColourStateRelationOn_executeBinary colour valid colourZero
          source target liveAfter .sub name left right hname hleft hright
          hrelation hcolourNonzero hnoAlias
  | and =>
      refine ⟨execute source (.and ⟨name, hname⟩ ⟨left, hleft⟩ ⟨right, hright⟩),
        execute target (.and ⟨colour name, valid name hname⟩
          ⟨colour left, valid left hleft⟩ ⟨colour right, valid right hright⟩),
        ?_, ?_, ?_⟩
      · simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,
          registerOfNat, hname, hleft, hright, executeInstructions]
      · simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          ...
          executeInstructions]
      · exact wordColourStateRelationOn_executeBinary colour valid colourZero
          source target liveAfter .and name left right hname hleft hright
          hrelation hcolourNonzero hnoAlias
  | or =>
      refine ⟨execute source (.or ⟨name, hname⟩ ⟨left, hleft⟩ ⟨right, hright⟩),
        execute target (.or ⟨colour name, valid name hname⟩
          ⟨colour left, valid left hleft⟩ ⟨colour right, valid right hright⟩),
        ?_, ?_, ?_⟩
      · simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,
          registerOfNat, hname, hleft, hright, executeInstructions]
      · simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          ...
          executeInstructions]
      · exact wordColourStateRelationOn_executeBinary colour valid colourZero
          source target liveAfter .or name left right hname hleft hright
          hrelation hcolourNonzero hnoAlias
  | xor =>
      refine ⟨execute source (.xor ⟨name, hname⟩ ⟨left, hleft⟩ ⟨right, hright⟩),
        execute target (.xor ⟨colour name, valid name hname⟩
          ⟨colour left, valid left hleft⟩ ⟨colour right, valid right hright⟩),
        ?_, ?_, ?_⟩
      · simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,
          registerOfNat, hname, hleft, hright, executeInstructions]
      · simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          ...
          executeInstructions]
      · exact wordColourStateRelationOn_executeBinary colour valid colourZero
          source target liveAfter .xor name left right hname hleft hright
          hrelation hcolourNonzero hnoAlias
