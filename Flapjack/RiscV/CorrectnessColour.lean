...
/-- The hardware register image of an internal CakeML stack register. -/
macro "hwRegister" n:term:max h:term:max : term =>
  `((⟨riscvRegisterName $n, riscvRegisterName_lt_32 $h⟩ : Fin 32))
...
structure WordColourStateRelation (colour : Nat → Nat) [NeZero width]
  ...
  register : ∀ (name : Nat) (hname : name < 32)
    ...
    readRegister source ((hwRegister name hname)) =
      readRegister target (hwRegister (colour name) hcolour)
...
theorem wordColourStateRelation_writeRegister
    ...
    WordColourStateRelation colour
      (writeRegister source ((hwRegister name hname)) value)
      (writeRegister target ((hwRegister (colour name) (valid name hname))) targetValue) := by
  by_cases hnameZero : riscvRegisterName name = 0
  · have hsourceZero : (hwRegister name hname) = 0 := Fin.ext hnameZero
    have hname27 : name = 27 :=
      riscvRegisterName_injective_lt_32 hname (by decide) (by simpa using hnameZero)
    have htargetZero : (hwRegister (colour name) (valid name hname)) = 0 := by
      apply Fin.ext
      simp [hname27, colourZero]
    rw [writeRegister, if_pos hsourceZero, writeRegister, if_pos htargetZero]
    exact hrelation
  · have hsourceNonzero : (hwRegister name hname) ≠ 0 := by
      intro h
      exact hnameZero (congrArg Fin.val h)
    have hcolourNonzero : riscvRegisterName (colour name) ≠ 0 := by
      intro h
      have hcolour27 : colour name = 27 :=
        riscvRegisterName_injective_lt_32 (valid name hname) (by decide)
          (by simpa using h)
      have hname27 : name = 27 := injective (by rw [hcolour27, colourZero])
      exact hnameZero (by rw [hname27, riscvRegisterName_twentySeven])
    have htargetNonzero : (hwRegister (colour name) (valid name hname)) ≠ 0 := by
      intro h
    ...
    · intro current hcurrent hcolour
      ...
      · subst current
        have hsourceEq : hwRegister name hcurrent = (hwRegister name hname) := by
          apply Fin.ext
          rfl
        have htargetEq : hwRegister (colour name) hcolour =
            (hwRegister (colour name) (valid name hname)) := by
          apply Fin.ext
          rfl
        simp only [writeRegister, hsourceNonzero, htargetNonzero, if_false,
          readRegister]
        ...
        exact hvalue
      · have htargetSame :
            hwRegister (colour current) hcolour ≠
              (hwRegister (colour name) (valid name hname)) := by
          apply riscvRegisterName_fin_ne
          · exact hcolour
          · exact valid name hname
          · intro h
            exact hsame (injective h)
        have hsourceCurrent : hwRegister current hcurrent ≠ (hwRegister name hname) := by
...
theorem wordColourStateRelation_executeAddi
    ...
    WordColourStateRelation colour
      (execute source (.addi (hwRegister name hname) (hwRegister sourceName hsource) 0))
      (execute target
        (.addi (hwRegister (colour name) (valid name hname))
          (hwRegister (colour sourceName) (valid sourceName hsource)) 0)) := by
  have hnext := wordColourStateRelation_nextPc colour valid source target hrelation
  have hvalue :
      readRegister source (hwRegister sourceName hsource) + 0 =
        readRegister target (hwRegister (colour sourceName) (valid sourceName hsource)) + 0 := by
  ...
  simpa [execute] using
    (wordColourStateRelation_writeRegister colour valid injective colourZero
      ...
      hnext name hname
      (readRegister source (hwRegister sourceName hsource) + 0)
      (readRegister target (hwRegister (colour sourceName) (valid sourceName hsource)) + 0)
      hvalue)
...
theorem evalWordProg_assignVar_applyColour
  ...
  have hsourceEval :
      evalWordProg source (.assign name (.var sourceName)) =
        some (execute source (.addi (hwRegister name hname) (hwRegister sourceName hsource) 0)) := by
    simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,       hname, hsource, executeInstructions]
  have htargetEval :
      evalWordProg target
    ...
    simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,       valid name hname, valid sourceName hsource,
      executeInstructions]
...
theorem evalWordProg_assignConst_applyColour
  ...
  have hread : readRegister source 0 = readRegister target 0 := by
    have h := hrelation.register 27 (by omega) (by omega)
  ...
  have hsourceEval :
      evalWordProg source (.assign name (.const value)) =
        some (execute source (.addi (hwRegister name hname) 0 value)) := by
    simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,       hname, executeInstructions]
  have htargetEval :
      evalWordProg target
          (wordApplyColour colour (.assign name (.const value))) =
        some (execute target
          (.addi (hwRegister (colour name) (valid name hname)) 0 value)) := by
    simp [wordApplyColour, wordApplyColourExp, evalWordProg,
      wordExpToInstructions, wordExpToInstruction,       valid name hname, executeInstructions]
...
theorem wordColourStateRelation_executeBinary
    ...
    WordColourStateRelation colour
      (execute source (match operator with
        | .add => .add (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright)
        | .sub => .sub (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright)
        | .and => .and (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright)
        | .or => .or (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright)
        | .xor => .xor (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright)))
      (execute target (match operator with
        | .add => .add (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright))
        | .sub => .sub (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright))
        | .and => .and (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright))
        | .or => .or (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright))
        | .xor => .xor (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright)))) := by
  cases operator with
  | add =>
      have hvalue :
          readRegister source (hwRegister left hleft) + readRegister source (hwRegister right hright) =
            readRegister target (hwRegister (colour left) (valid left hleft)) +
              readRegister target (hwRegister (colour right) (valid right hright)) := by
      ...
      simpa [execute] using
        (wordColourStateRelation_writeRegister colour valid injective colourZero
          ...
          (readRegister source (hwRegister left hleft) + readRegister source (hwRegister right hright))
          (readRegister target (hwRegister (colour left) (valid left hleft)) +
            readRegister target (hwRegister (colour right) (valid right hright))) hvalue)
  | sub =>
      have hvalue :
          readRegister source (hwRegister left hleft) - readRegister source (hwRegister right hright) =
            readRegister target (hwRegister (colour left) (valid left hleft)) -
  ...
  | add =>
      have hsourceEval :
          evalWordProg source (.assign name (.op .add [.var left, .var right])) =
            some (execute source (.add (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright))) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hleft, hright, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
                (.assign name (.op .add [.var left, .var right]))) =
            some (execute target
              (.add (hwRegister (colour name) (valid name hname))
                (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright)))) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid left hleft,
  ...
  | sub =>
      have hsourceEval :
          evalWordProg source (.assign name (.op .sub [.var left, .var right])) =
            some (execute source (.sub (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright))) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hleft, hright, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.sub (hwRegister (colour name) (valid name hname))
                (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright)))) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid left hleft,
          valid right hright, executeInstructions]
  ...
  | and =>
      have hsourceEval :
          evalWordProg source (.assign name (.op .and [.var left, .var right])) =
            some (execute source (.and (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright))) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hleft, hright, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.and (hwRegister (colour name) (valid name hname))
                (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright)))) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid left hleft,
          valid right hright, executeInstructions]
  ...
  | or =>
      have hsourceEval :
          evalWordProg source (.assign name (.op .or [.var left, .var right])) =
            some (execute source (.or (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright))) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hleft, hright, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.or (hwRegister (colour name) (valid name hname))
                (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright)))) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid left hleft,
          valid right hright, executeInstructions]
  ...
  | xor =>
      have hsourceEval :
          evalWordProg source (.assign name (.op .xor [.var left, .var right])) =
            some (execute source (.xor (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright))) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hleft, hright, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.xor (hwRegister (colour name) (valid name hname))
                (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright)))) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid left hleft,
          valid right hright, executeInstructions]
...
theorem wordColourStateRelation_executeImmediateBinary
    ...
    WordColourStateRelation colour
      (execute source (match operator with
        | .add => .addi (hwRegister name hname) (hwRegister sourceName hsource) value
        | .sub => .addi (hwRegister name hname) (hwRegister sourceName hsource) (0 - value)
        | .and => .andi (hwRegister name hname) (hwRegister sourceName hsource) value
        | .or => .ori (hwRegister name hname) (hwRegister sourceName hsource) value
        | .xor => .xori (hwRegister name hname) (hwRegister sourceName hsource) value))
      (execute target (match operator with
        | .add => .addi (hwRegister (colour name) (valid name hname))
            (hwRegister (colour sourceName) (valid sourceName hsource)) value
        | .sub => .addi (hwRegister (colour name) (valid name hname))
            (hwRegister (colour sourceName) (valid sourceName hsource)) (0 - value)
        | .and => .andi (hwRegister (colour name) (valid name hname))
            (hwRegister (colour sourceName) (valid sourceName hsource)) value
        | .or => .ori (hwRegister (colour name) (valid name hname))
            (hwRegister (colour sourceName) (valid sourceName hsource)) value
        | .xor => .xori (hwRegister (colour name) (valid name hname))
            (hwRegister (colour sourceName) (valid sourceName hsource)) value)) := by
  cases operator with
  | add =>
      have hvalue :
          readRegister source (hwRegister sourceName hsource) + value =
            readRegister target (hwRegister (colour sourceName) (valid sourceName hsource)) + value := by
        rw [hrelation.register sourceName hsource (valid sourceName hsource)]
      have hnext := wordColourStateRelation_nextPc colour valid source target hrelation
      simpa [execute] using
        (wordColourStateRelation_writeRegister colour valid injective colourZero
          ...
          hnext name hname
          (readRegister source (hwRegister sourceName hsource) + value)
          (readRegister target (hwRegister (colour sourceName) (valid sourceName hsource)) + value) hvalue)
  | sub =>
      have hvalue :
          readRegister source (hwRegister sourceName hsource) + (0 - value) =
            readRegister target (hwRegister (colour sourceName) (valid sourceName hsource)) +
              (0 - value) := by
        rw [hrelation.register sourceName hsource (valid sourceName hsource)]
      have hnext := wordColourStateRelation_nextPc colour valid source target hrelation
      simpa [execute] using
  ...
  | add =>
      have hsourceEval :
          evalWordProg source (.assign name (.op .add [.var sourceName, .const value])) =
            some (execute source (.addi (hwRegister name hname) (hwRegister sourceName hsource) value)) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hsource, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.addi (hwRegister (colour name) (valid name hname))
                (hwRegister (colour sourceName) (valid sourceName hsource)) value)) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid sourceName hsource,
          executeInstructions]
      exact ⟨_, _, hsourceEval, htargetEval,
  ...
  | sub =>
      have hsourceEval :
          evalWordProg source (.assign name (.op .sub [.var sourceName, .const value])) =
            some (execute source (.addi (hwRegister name hname) (hwRegister sourceName hsource) (0 - value))) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hsource, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.addi (hwRegister (colour name) (valid name hname))
                (hwRegister (colour sourceName) (valid sourceName hsource)) (0 - value))) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid sourceName hsource,
          executeInstructions]
      exact ⟨_, _, hsourceEval, htargetEval,
  ...
  | and =>
      have hsourceEval :
          evalWordProg source (.assign name (.op .and [.var sourceName, .const value])) =
            some (execute source (.andi (hwRegister name hname) (hwRegister sourceName hsource) value)) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hsource, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.andi (hwRegister (colour name) (valid name hname))
                (hwRegister (colour sourceName) (valid sourceName hsource)) value)) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid sourceName hsource,
          executeInstructions]
  ...
  | or =>
      have hsourceEval :
          evalWordProg source (.assign name (.op .or [.var sourceName, .const value])) =
            some (execute source (.ori (hwRegister name hname) (hwRegister sourceName hsource) value)) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hsource, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.ori (hwRegister (colour name) (valid name hname))
                (hwRegister (colour sourceName) (valid sourceName hsource)) value)) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid sourceName hsource,
          executeInstructions]
  ...
  | xor =>
      have hsourceEval :
          evalWordProg source (.assign name (.op .xor [.var sourceName, .const value])) =
            some (execute source (.xori (hwRegister name hname) (hwRegister sourceName hsource) value)) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hsource, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.xori (hwRegister (colour name) (valid name hname))
                (hwRegister (colour sourceName) (valid sourceName hsource)) value)) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid sourceName hsource,
          executeInstructions]
...
theorem wordColourStateRelation_executeSllForRotate
    ...
    (hright : right < 32) :
    WordColourStateRelation colour
      (execute source (.sll (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright)))
      (execute target (.sll (hwRegister (colour name) (valid name hname))
        (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright)))) := by
  have hvalue :
      BitVec.shiftLeft (readRegister source (hwRegister left hleft))
          (shiftAmount (readRegister source (hwRegister right hright))) =
        BitVec.shiftLeft (readRegister target (hwRegister (colour left) (valid left hleft)))
          (shiftAmount (readRegister target (hwRegister (colour right) (valid right hright)))) := by
    rw [hrelation.register left hleft (valid left hleft),
      hrelation.register right hright (valid right hright)]
  have hnext := wordColourStateRelation_nextPc colour valid source target hrelation
  simpa [execute] using
    (wordColourStateRelation_writeRegister colour valid injective colourZero
      ...
      (BitVec.shiftLeft (readRegister source (hwRegister left hleft))
        (shiftAmount (readRegister source (hwRegister right hright))))
      (BitVec.shiftLeft (readRegister target (hwRegister (colour left) (valid left hleft)))
        (shiftAmount (readRegister target (hwRegister (colour right) (valid right hright))))) hvalue)
...
theorem wordColourStateRelation_executeSrlForRotate
    ...
    (hright : right < 32) :
    WordColourStateRelation colour
      (execute source (.srl (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright)))
      (execute target (.srl (hwRegister (colour name) (valid name hname))
        (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright)))) := by
  have hvalue :
      BitVec.ushiftRight (readRegister source (hwRegister left hleft))
          (shiftAmount (readRegister source (hwRegister right hright))) =
        BitVec.ushiftRight (readRegister target (hwRegister (colour left) (valid left hleft)))
          (shiftAmount (readRegister target (hwRegister (colour right) (valid right hright)))) := by
    rw [hrelation.register left hleft (valid left hleft),
      hrelation.register right hright (valid right hright)]
  have hnext := wordColourStateRelation_nextPc colour valid source target hrelation
...
theorem wordColourStateRelation_executeRotateRight
    ...
    WordColourStateRelation colour
      (executeInstructions source
        [.ori 31 0 (BitVec.ofNat width width), .sub 31 31 (hwRegister right hright),
          .sll 31 (hwRegister left hleft) 31, .srl (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright),
          .or (hwRegister name hname) (hwRegister name hname) 31])
      (executeInstructions target
        [.ori (hwRegister (colour 31) (valid 31 (by omega))) 0
            (BitVec.ofNat width width),
          .sub (hwRegister (colour 31) (valid 31 (by omega))) (hwRegister (colour 31) (valid 31 (by omega)))
            (hwRegister (colour right) (valid right hright)),
          .sll (hwRegister (colour 31) (valid 31 (by omega))) (hwRegister (colour left) (valid left hleft))
            (hwRegister (colour 31) (valid 31 (by omega))),
          .srl (hwRegister (colour name) (valid name hname)) (hwRegister (colour left) (valid left hleft))
            (hwRegister (colour right) (valid right hright)),
          .or (hwRegister (colour name) (valid name hname)) (hwRegister (colour name) (valid name hname))
            (hwRegister (colour 31) (valid 31 (by omega)))]) := by
  ...
  have h1 := wordColourStateRelation_executeImmediateBinary colour valid
    injective colourZero source target hrelation .or 31 27 (BitVec.ofNat width width)
      (by omega) (by omega)
  have h1' :
      WordColourStateRelation colour
  ...
  have h3' :
      WordColourStateRelation colour
        (execute
          (execute (execute source (.ori 31 0 (BitVec.ofNat width width)))
            (.sub 31 31 (hwRegister right hright)))
          (.sll 31 (hwRegister left hleft) 31))
        (execute
          (execute (execute target (.ori 31 0 (BitVec.ofNat width width)))
            (.sub 31 31 (hwRegister (colour right) (valid right hright))))
          (.sll 31 (hwRegister (colour left) (valid left hleft)) 31)) := by
  ...
  have h4 := wordColourStateRelation_executeSrlForRotate colour valid injective colourZero
    ...
    (execute
      (execute (execute target (.ori 31 0 (BitVec.ofNat width width)))
        (.sub 31 31 (hwRegister (colour right) (valid right hright))))
      (.sll 31 (hwRegister (colour left) (valid left hleft)) 31)) h3' name left right
      hname hleft hright
  have h4' :
      WordColourStateRelation colour
        (execute
          (execute
            (execute (execute source (.ori 31 0 (BitVec.ofNat width width)))
              (.sub 31 31 (hwRegister right hright)))
            (.sll 31 (hwRegister left hleft) 31))
          (.srl (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright)))
        (execute
          (execute
            (execute (execute target (.ori 31 0 (BitVec.ofNat width width)))
              (.sub 31 31 (hwRegister (colour right) (valid right hright))))
            (.sll 31 (hwRegister (colour left) (valid left hleft)) 31))
          (.srl (hwRegister (colour name) (valid name hname)) (hwRegister (colour left) (valid left hleft))
            (hwRegister (colour right) (valid right hright)))) := by
  ...
  have h5 := wordColourStateRelation_executeBinary colour valid injective colourZero
    ...
    (execute
      (execute
        ...
        (.sll 31 (hwRegister (colour left) (valid left hleft)) 31))
      (.srl (hwRegister (colour name) (valid name hname)) (hwRegister (colour left) (valid left hleft))
        (hwRegister (colour right) (valid right hright)))) h4' .or name name 31
  ...
  have hsourceEval :
      evalWordProg source (.assign name (.shift .ror (.var left) (.var right))) =
        some (executeInstructions source
          [.ori 31 0 (BitVec.ofNat width width), .sub 31 31 (hwRegister right hright),
            .sll 31 (hwRegister left hleft) 31, .srl (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright),
            .or (hwRegister name hname) (hwRegister name hname) 31]) := by
    simp [evalWordProg, wordExpToInstructions,       hname, hleft, hright, hnameScratch, hleftScratch,
      ...
      evalWordProg target
        ...
        some (executeInstructions target
          [.ori 31 0 (BitVec.ofNat width width),
            ...
            .srl (hwRegister (colour name) (valid name hname)) (hwRegister (colour left) (valid left hleft))
              (hwRegister (colour right) (valid right hright)),
            .or (hwRegister (colour name) (valid name hname)) (hwRegister (colour name) (valid name hname)) 31]) := by
    simp [evalWordProg, wordApplyColour, wordApplyColourExp, wordExpToInstructions,
...
theorem wordColourStateRelation_executeShiftImmediate
    ...
    WordColourStateRelation colour
      (execute source (match operator with
        | .lsl => .slli (hwRegister name hname) (hwRegister left hleft) amount
        | .lsr => .srli (hwRegister name hname) (hwRegister left hleft) amount
        | .asr => .srai (hwRegister name hname) (hwRegister left hleft) amount
        | .ror => .slli (hwRegister name hname) (hwRegister left hleft) amount))
      (execute target (match operator with
        | .lsl => .slli (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) amount
        | .lsr => .srli (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) amount
        | .asr => .srai (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) amount
        | .ror => .slli (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) amount)) := by
  cases operator with
  | lsl =>
      have hvalue :
          BitVec.shiftLeft (readRegister source (hwRegister left hleft)) (shiftAmount amount) =
            BitVec.shiftLeft (readRegister target (hwRegister (colour left) (valid left hleft)))
  ...
  | lsr =>
      have hvalue :
          BitVec.ushiftRight (readRegister source (hwRegister left hleft)) (shiftAmount amount) =
            BitVec.ushiftRight (readRegister target (hwRegister (colour left) (valid left hleft)))
              (shiftAmount amount) := by
        rw [hrelation.register left hleft (valid left hleft)]
      have hnext := wordColourStateRelation_nextPc colour valid source target hrelation
  ...
  | asr =>
      have hvalue :
          BitVec.sshiftRight (readRegister source (hwRegister left hleft)) (shiftAmount amount) =
            BitVec.sshiftRight (readRegister target (hwRegister (colour left) (valid left hleft)))
              (shiftAmount amount) := by
        rw [hrelation.register left hleft (valid left hleft)]
      have hnext := wordColourStateRelation_nextPc colour valid source target hrelation
      simpa [execute] using
  ...
  | lsl =>
      have hsourceEval :
          evalWordProg source (.assign name (.shift .lsl (.var left) (.const amount))) =
            some (execute source (.slli (hwRegister name hname) (hwRegister left hleft) amount)) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hleft, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
                (.assign name (.shift .lsl (.var left) (.const amount)))) =
            some (execute target
              (.slli (hwRegister (colour name) (valid name hname))
                (hwRegister (colour left) (valid left hleft)) amount)) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid left hleft, executeInstructions]
      exact ⟨_, _, hsourceEval, htargetEval,
  ...
  | lsr =>
      have hsourceEval :
          evalWordProg source (.assign name (.shift .lsr (.var left) (.const amount))) =
            some (execute source (.srli (hwRegister name hname) (hwRegister left hleft) amount)) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hleft, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.srli (hwRegister (colour name) (valid name hname))
                (hwRegister (colour left) (valid left hleft)) amount)) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid left hleft, executeInstructions]
      exact ⟨_, _, hsourceEval, htargetEval,
  ...
  | asr =>
      have hsourceEval :
          evalWordProg source (.assign name (.shift .asr (.var left) (.const amount))) =
            some (execute source (.srai (hwRegister name hname) (hwRegister left hleft) amount)) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hleft, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.srai (hwRegister (colour name) (valid name hname))
                (hwRegister (colour left) (valid left hleft)) amount)) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid left hleft, executeInstructions]
      exact ⟨_, _, hsourceEval, htargetEval,
...
theorem wordColourStateRelation_executeShift
    ...
    WordColourStateRelation colour
      (execute source (match operator with
        | .lsl => .sll (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright)
        | .lsr => .srl (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright)
        | .asr => .sra (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright)
        | .ror => .sll (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright)))
      (execute target (match operator with
        | .lsl => .sll (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright))
        | .lsr => .srl (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright))
        | .asr => .sra (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright))
        | .ror => .sll (hwRegister (colour name) (valid name hname))
            (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright)))) := by
  cases operator with
  | lsl =>
      have hvalue :
  ...
  | asr =>
      have hvalue :
          BitVec.sshiftRight (readRegister source (hwRegister left hleft))
              (shiftAmount (readRegister source (hwRegister right hright))) =
            BitVec.sshiftRight (readRegister target (hwRegister (colour left) (valid left hleft)))
              (shiftAmount (readRegister target (hwRegister (colour right) (valid right hright)))) := by
        rw [hrelation.register left hleft (valid left hleft),
  ...
  | lsl =>
      have hsourceEval :
          evalWordProg source (.assign name (.shift .lsl (.var left) (.var right))) =
            some (execute source (.sll (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright))) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hleft, hright, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.sll (hwRegister (colour name) (valid name hname))
                (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright)))) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid left hleft,
          valid right hright, executeInstructions]
  ...
  | lsr =>
      have hsourceEval :
          evalWordProg source (.assign name (.shift .lsr (.var left) (.var right))) =
            some (execute source (.srl (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright))) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hleft, hright, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.srl (hwRegister (colour name) (valid name hname))
                (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright)))) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid left hleft,
          valid right hright, executeInstructions]
  ...
  | asr =>
      have hsourceEval :
          evalWordProg source (.assign name (.shift .asr (.var left) (.var right))) =
            some (execute source (.sra (hwRegister name hname) (hwRegister left hleft) (hwRegister right hright))) := by
        simp [evalWordProg, wordExpToInstructions, wordExpToInstruction,           hname, hleft, hright, executeInstructions]
      have htargetEval :
          evalWordProg target
              (wordApplyColour colour
            ...
            some (execute target
              (.sra (hwRegister (colour name) (valid name hname))
                (hwRegister (colour left) (valid left hleft)) (hwRegister (colour right) (valid right hright)))) := by
        simp [evalWordProg, wordApplyColour, wordApplyColourExp,
          wordExpToInstructions, wordExpToInstruction, valid name hname, valid left hleft,
          valid right hright, executeInstructions]
...
theorem evalWordProg_moveOne_applyColour [NeZero width]
  ...
  have hsourceMove :
    ...
    simp [evalWordProg, wordMoveToInstructions, wordMoveToInstructionsAux,
      ...
      wordExpToInstruction, hname, hsource, hname31, hsource31,
      hne']
  have htargetMove :
      evalWordProg target
    ...
    simp [evalWordProg, wordApplyColour, wordMoveToInstructions,
      ...
      wordExpToInstructions, wordExpToInstruction, valid name hname, valid sourceName hsource, hcolourName31,
...
theorem evalWordProg_moveTwo_applyColour [NeZero width]
  ...
  have hsourceMove :
    ...
    simp [evalWordProg, wordMoveToInstructions, wordMoveToInstructionsAux,
      wordMoveRegisterDestinations, wordMoveRegisterReady,
      wordMoveRegisterRemoveDestination, wordExpToInstructions,
      wordExpToInstruction, hdestinationOne, hsourceOne,
  ...
  have htargetMove :
    ...
    simp [evalWordProg, wordApplyColour, wordMoveToInstructions,
      wordMoveToInstructionsAux, wordMoveRegisterDestinations,
      wordMoveRegisterReady, wordMoveRegisterRemoveDestination,
      wordExpToInstructions, wordExpToInstruction, valid destinationOne hdestinationOne, valid sourceOne hsourceOne,
...
theorem wordAllocateProgramWithClashTreeAndColour_straightLine_simulation
    ...
    (colourZero : wordFindVar context 27 = 27)
...
theorem wordAllocateGraphProgram_straightLine_simulation
  ...
  have hzero : wordGraphColouringAt hallocation.colouring 27 = 27 := by
...
theorem wordAllocateGraphFunction_straightLine_simulation
  ...
  have hzero : wordGraphColouringAt a.colouring 27 = 27 := by
...
theorem wordColourStateRelation_evalWordCondition [NeZero width]
  ...
  | imm value =>
      ...
      cases operator <;>
        simp [evalWordCondition, wordApplyColourRegImm, hcondition, hcondition', hconditionValue]
  | reg right =>
      ...
      have hrightValue := hrelation.register right hright'
        (valid right hright')
      cases operator <;>
        simp [evalWordCondition, wordApplyColourRegImm, hcondition, hright', hcondition', hrightColour, hleftValue,
          hrightValue]
...
theorem evalWordFunction_wordVarStraightLine_return_applyColour [NeZero width]
    ...
    | cons name values ih =>
        ...
        have hhead :
            ...
            (do
              ...
              pure (readRegister target' register)) := by
          simp [labRegisterOfNat_of_lt_32 hname, labRegisterOfNat_of_lt_32 hcolour,
            hregister]
  ...
  have hmapSome : ∀ (state : State width) (names : List Nat),
      ...
      ∃ result, List.mapM (fun name => do
        let register ← labRegisterOfNat name
        pure (readRegister state register)) names = some result := by
    intro state names
    ...
    | cons name names ih =>
        ...
        have hregister : labRegisterOfNat name = some (hwRegister name hname) :=
          labRegisterOfNat_of_lt_32 hname
        rcases ih htail with ⟨result, hresult⟩
        refine ⟨readRegister state (hwRegister name hname) :: result, ?_⟩
  ...
  · simp only [evalWordFunction]
    ...
    have hsourceValues' :
        values.mapM (fun name =>
          (labRegisterOfNat name).bind (fun register =>
            some (readRegister source' register))) =
  ...
  · refine ⟨?_, hrelation'⟩
    ...
    have htargetValues' :
        (values.map colour).mapM (fun name =>
          (labRegisterOfNat name).bind (fun register =>
            some (readRegister target' register))) =
          some sourceValues := by
      simpa [Option.map, Option.bind] using htargetValues
    have htargetValues'' :
        List.mapM ((fun name =>
          (labRegisterOfNat name).bind (fun register =>
            some (readRegister target' register))) ∘ colour) values =
          some sourceValues := by
...
theorem wordAllocateGraphFunctionWithEntry_riscv_return_simulation
  ...
  have hzero : wordGraphColouringAt actualAllocation.colouring 27 = 27 := by
...
theorem evalWordCallWithHandlersAndFfi_raiseHandler_applyColour_general
    ...
    (hsourceHandlerRegister : labRegisterOfNat handlerName =
      some (hwRegister handlerName hsourceHandlerName))
    (htargetHandlerRegister : labRegisterOfNat (colour handlerName) =
      some (hwRegister (colour handlerName) (valid handlerName hsourceHandlerName)))
    (hhandlerSource : evalWordFunctionWithHandlersAndFfi sourceFunctions
      ...
      (writeRegister
        { sourceState with
          ...
          privilege := sourceBodyState.privilege
          mode := sourceBodyState.mode }
        (hwRegister handlerName hsourceHandlerName)
    ...
    (hhandlerTarget : evalWordFunctionWithHandlersAndFfi targetFunctions
      ...
      (writeRegister
        { targetState with
          ...
          privilege := targetBodyState.privilege
          mode := targetBodyState.mode }
        (hwRegister (colour handlerName) (valid handlerName hsourceHandlerName))
  ...
  have hsourceCall := evalWordCallWithHandlersAndFfi_raise_handler_of_eval
    ...
    (hwRegister handlerName hsourceHandlerName) (.returned sourceHandlerState sourceValues)
    hlookupSource hargumentsSource hbindSource hbodySource
  ...
  have htargetCall := evalWordCallWithHandlersAndFfi_raise_handler_of_eval
    ...
    targetHandlerBody (hwRegister (colour handlerName) (valid handlerName hsourceHandlerName))
