...
/-! The compiler-facing form packages the layout equation together with the
    coloured data simulation, mirroring the existing uncoloured conditional
    contract. -/

theorem wordFunctionToRiscVWithCalls_ite_coloured_data_relation
    [NeZero width]
    (colour : Nat → Nat)
  ...
  have hsimulation :=
    evalWordFunction_ite_executeCodeUntil_of_coloured_data_relation
      (colour := colour) (state := state) (operator := operator)
      (condition := condition) (source := source)
      (thenBranch := thenBranch) (elseBranch := elseBranch)
      (branchLeft := branchLeft) (right := right) (thenCode := thenCode)
      (elseCode := elseCode) (returns := returnValues) hpc hzero hoperands
      hthen helse hthenNonbranching helseNonbranching hbound
