...
theorem wordSsaRenameInst_store {α : Type u}
    (ssa : WordSsaState) (source address : Nat) :
    wordSsaRenameInst ssa (.mem .store source address : WordInst α) =
...
theorem wordSsaRenameInst_load {α : Type u}
    (ssa : WordSsaState) (destination address : Nat) :
    wordSsaRenameInst ssa (.mem .load destination address : WordInst α) =
...
theorem wordSsaRenameInst_div {α : Type u}
    (ssa : WordSsaState) (destination dividend divisor : Nat) :
    wordSsaRenameInst ssa (.arith (.div destination dividend divisor) : WordInst α) =
      ((wordSsaFresh ssa destination).1,
...
theorem wordSsaRenameInst_longMul {α : Type u}
    (ssa : WordSsaState) (destinationLeft destinationRight sourceLeft sourceRight : Nat) :
    wordSsaRenameInst ssa
        (.arith (.longMul destinationLeft destinationRight sourceLeft sourceRight) : WordInst α) =
...
theorem wordSsaRenameInst_addCarry {α : Type u}
    (ssa : WordSsaState) (destination resultCarry sourceLeft sourceRight carryIn : Nat) :
    wordSsaRenameInst ssa
        (.arith (.addCarry destination resultCarry sourceLeft sourceRight carryIn) :
          WordInst α) =
      (let sourceLeft := wordSsaRead ssa sourceLeft
       let sourceRight := wordSsaRead ssa sourceRight
       let carryIn := wordSsaRead ssa carryIn
       let (ssa, freshDestination) := wordSsaFresh ssa destination
       let (ssa, freshCarry) := wordSsaFresh ssa resultCarry
       (ssa, .arith (.addCarry freshDestination freshCarry
         sourceLeft sourceRight carryIn))) := by
  rfl
...
theorem evalWordProg_ssaRename_longMul_destinations [NeZero width]
  ...
  simp only [wordSsaRenameInst_longMul, hfirst, hsecond]
...
theorem evalWordProg_ssaRename_addCarry_destinations [NeZero width]
  ...
  simp only [wordSsaRenameInst_addCarry, hfirst, hsecond]
  let sourceCode : List (Instruction width) :=
