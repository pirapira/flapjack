...
def wordDeadCodeAuxWithLabels : WordProg α → List Nat → List (List Nat × List Nat) →
  ...
  | .call returns target arguments handler, live, _, _ =>
      (.call returns target arguments handler,
        wordDeadAddReads live (wordProgReadVarsFast
          (.call returns target arguments handler)))
  | .alloc destination cutsets, live, _, _ =>
  ...
  | .opCurrHeap operator destination source, live, _, _ =>
      if destination ∈ live then
        ...
        (.skip, live)
  | .install codeBuffer codeLength dataBuffer dataLength cutsets, live, _, _ =>
  ...
  | .ffi function configuration configurationLength array arrayLength liveSet, live, _, _ =>
