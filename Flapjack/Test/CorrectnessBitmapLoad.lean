...
def bitmapLoadTestSource : WordStackMachineState 64 :=
  { registers := fun _ => 0
    stack := fun _ => 0
    stores := fun _ => 0
    memory := fun _ => 0
...
example :
    (executeInstructions (zeroState 64)
        [.addi 29 0 (BitVec.ofNat 64 88), .sub 29 10 29,
         .loadWord 6 29, .add 6 6 7, .addi 31 0 (BitVec.ofNat 64 3),
         .sll 6 6 31, .loadWord 6 6]).registers 6 =
      bitmapLoadTestSource.memory
        ((bitmapLoadTestSource.memory
            (bitmapLoadTestSource.registers bitmapLoadTestConfig.storeBase -
              BitVec.ofNat 64
                (bitmapLoadTestConfig.bytesInWord * stackStorePosition .bitmapBase)) +
          bitmapLoadTestSource.registers 7) <<<
          shiftAmount (BitVec.ofNat 64 bitmapLoadTestConfig.wordShift)) := by
  apply compileStackProgramNatToRiscV_bitmapLoad_source_simulation
    (context := { services := [] }) (config := bitmapLoadTestConfig)
    (sectionId := 2) (initialLabel := 3) (destination := 6) (address := 7)
