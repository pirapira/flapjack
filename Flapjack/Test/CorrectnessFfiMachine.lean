...
/- The generalized counted contract also accepts a compiler result carrying
   return-register metadata.  The count concerns the emitted code, so it
   should not require the compiler to erase that metadata first. -/
