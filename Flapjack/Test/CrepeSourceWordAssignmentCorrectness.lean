...
theorem source_word_assignment_temporary_bridge :
    ∃ result,
      evalCrepFullProg [] (fun _ _ => none) (fun _ _ _ _ _ _ => none)
        (fun _ _ _ _ => none) 0 0 3 sourceWordAssignmentState
        (compileProg sourceWordAssignmentContext
          (.assign .local "x"
            (SourceWordExp.op .add (SourceWordExp.«local» "x")
              (SourceWordExp.const 1)).toExp)) = some result := by
  have hrel : panValueCrepStateRel [] sourceWordAssignmentContext
      (fun name => if name == "x" then some (.word 3) else none)
      (fun _ => none) (fun _ => none) sourceWordAssignmentState := by
    refine ⟨rfl, ?_, rfl⟩
    ...
    simp [sourceWordAssignmentState, readCrepLocals, panValueShape,
      panShapeMatches, panValueFlatWords, panValueFlatWordsFuel]
  have h := compile_full_pan_value_local_assign_source_word_temporary_relation
    (α := Nat) (context := sourceWordAssignmentContext) (structs := [])
    (sourceFunctions := []) (functions := [])
    (sourceLocals := fun name => if name == "x" then some (.word 3) else none)
    (sourceGlobals := fun _ => none) (sourceMemory := fun _ => none)
    (state := sourceWordAssignmentState) (primitive := fun _ _ => none)
    ...
    (baseAddress := 0) (topAddress := 0) (bytesInWord := 8) (name := "x")
    (slot := 0) (temporary := 2) (oldValue := 3) (value := 4)
    (expression := .op .add (.«local» "x") (.const 1))
    (compiled := .op .add [.var 0, .const 1])
    (hlookup := by simp [sourceWordAssignmentContext, lookupInfo])
    (hold := by simp) (hsource := by
      simp [SourceWordExp.toExp, evalPanValueExp,
        evalPanValueExp.evalPanValueExps, evalPanBinOp])
    (hcompile := by
      simp [sourceWordAssignmentContext, SourceWordExp.toExp, compileExp,
        compileExp.compileExpList, cexpHeads, lookupInfo])
    (hcompiled := by
      simp [evalCrepFullExp, sourceWordAssignmentState, evalPanBinOp])
    (hnotDistinct := by
      simp [distinctLists, crepExpVars, crepExpVars.crepExpVarsList]) (hfresh := rfl)
    (htemporary := by simp [sourceWordAssignmentState])
    (htemporaryNe := by decide) (hrel := hrel)
    (hnoalias := by
      intro oldName oldShape oldSlots hne hlookup
      simp [sourceWordAssignmentContext, lookupInfo] at hlookup
      rcases hlookup with ⟨hname, _, _⟩
