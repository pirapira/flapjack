...
def runChecks : IO Bool := do
  let directPassed := machineResult == originalProbeResult
  let _ ← if directPassed then
  ...
  pure (directPassed && multiplicationPassed)
