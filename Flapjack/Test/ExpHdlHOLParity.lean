...
/-!
# Exact-carrier parity for `pan_to_crep$exp_hdl`

The expected values come from the direct HOL-EVAL fixture
`scripts/hol-probes/exp_hdl_probe.out`, sourced from
`cakeml/pancake/pan_to_crepScript.sml:106-112`.  `expHdlHOL` is the exact port
over the `mlstring`-backed `ShapeHOL` finite map and the width-indexed
`CrepProgHOL`; it is tagged `@[hol ... exp_hdl_def]` and must reproduce the HOL
rows on the exact carriers.  The duplicate fixtures pin the finite-map
semantics: the last binding wins for the HOL `FLOOKUP` map.
...
/-- The kernel bridge reproduces the exact carrier on a duplicate-bearing
    production map, so the executed `expHdlFiniteMap` and the tagged
