...
example {width : Nat} [ShiftLeft (BitVec width)] [ShiftRight (BitVec width)]
    ...
    ∀ output, output ∈ globalCompileTopCake declarations start →
      ∀ function, output = .function function →
        function.params.all (fun parameter => isWfShapeNil parameter.2) = true ∧
          isWfShapeNil function.returnShape = true :=
  globalCompileTopCake_shapes_wf_nil state declarations start state'
    heval hstructs hadmissible
