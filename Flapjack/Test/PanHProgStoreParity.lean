...
def observeReturn : Bool :=
  ...
  | some (.ret .normal, world) => world.state == 0
  | _ => false

def observeMismatch : Bool :=
  match compFfi 1 zeroTree shortWorld with
  | some (.ret (.finalFfi event), _) =>
      event.bytes == [7, 3] && event.outcome == .failed
  | _ => false
