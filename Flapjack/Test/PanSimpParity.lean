...
#check @Flapjack.PanValueFfiClockNormalAdequateProgAt_while
...
#check @Flapjack.evalPanValueFfiClockProg_tick_zero_timeout_ioEvents_prefix
#check @Flapjack.evalPanValueFfiClockProg_tick_succ_ioEvents_prefix
#check @Flapjack.evalPanValueFfiClockProg_while_zero_ioEvents_prefix
#check @Flapjack.evalPanValueFfiClockProg_ite_true_ioEvents_prefix
#check @Flapjack.evalPanValueFfiClockProg_ite_false_ioEvents_prefix
#check @Flapjack.evalPanValueFfiClockProg_return_ioEvents_prefix
#check @Flapjack.evalPanValueFfiClockProg_raise_ioEvents_prefix
#check @Flapjack.panResultFfi_panValueFfiClockRestoreLocal
#check @Flapjack.evalPanValueFfiClockProg_dec_ioEvents_prefix
...
#check @Flapjack.evalPanValueFfiClockProgram_of_declarations_and_returned_call_result_rel_with_context_code_ioEvents_prefix
...
#check @Flapjack.evalPanValueFfiClockProgram_of_declarations_and_normal_call_context_code
#check @Flapjack.evalPanValueFfiClockProgram_of_declarations_and_returned_call_context_code
#check @Flapjack.evalPanValueFfiClockProgram_of_declarations_and_timeout_call_context_code
...
#check @Flapjack.evalPanValueFfiClockProgram_of_declarations_and_normal_raised_timeout_call_result_rel
#check @Flapjack.evalPanValueFfiClockProgram_of_declarations_and_normal_raised_timeout_call_result_rel_ioEvents_prefix
...
#check @Flapjack.evalPanValueFfiClockProgram_of_related_declarations_and_timeout_call_context_code_adequacy
#check @Flapjack.evalPanValueFfiClockProgram_of_related_declarations_and_timeout_call_result_rel_adequacy_with_context
...
#check @Flapjack.evalPanValueFfiClockProgram_of_related_declarations_and_returned_call_context_code_adequacy
...
#check @Flapjack.evalPanValueFfiClockProgram_of_related_declarations_and_finalFfi_call_result_rel_adequacy_with_context
#check @Flapjack.evalPanValueFfiClockProgram_of_related_declarations_and_finalFfi_call_context_code_adequacy
#check @Flapjack.evalPanValueFfiClockProgram_of_related_declarations_and_finalFfi_call_result_rel_adequacy

/-! The normal-call context-code boundary keeps the concrete evaluator and
    source/target state relation visible for the Cake `state_rel` induction. -/
theorem normal_call_context_code_fixture
  ...
  exact evalPanValueFfiClockProgram_of_declarations_and_normal_call_context_code
    ...
    hdeclarations hentry hcall hstate

/-! Typed direct returned-call regression for the context-code state boundary. -/
theorem returned_call_context_code_fixture
    (initial : PanValueFfiProgramState Nat Unit)
    (clock : Nat) (fuel : Nat)
    (declarations : List (Decl Nat)) (entry : FunName)
    (arguments : List (Exp Nat))
    (state : PanValueProgramState Nat) (shape : Shape)
    (locals globals : VarName → Option (PanValue Nat))
    (memory : Nat → Option (PanValue Nat)) (ffi : FfiState Unit)
    (value : PanValue Nat) (nextClock : Nat)
    (pcContext : CompileContext Nat)
    (exceptionRel : ExceptionId → PanValue Nat → Nat → Prop)
    (exceptionCode : ExceptionId → Option Nat)
    (globalsLookup : CrepState Nat → PanValue Nat → Option (List Nat))
    (targetState : CrepState Nat) (targetValues : List Nat)
    (memoryAccess : Option (PanValueMemoryAccess Nat))
    (memoryHandler : Option (PanValueMemoryFfiHandler Nat Unit))
    (hdeclarations : evalPanValueDeclarations initial.source declarations
      (memoryAccess := memoryAccess) = some state)
    ...
    evalPanValueFfiClockProgram evaluatorContext initial clock
      ...
      some (.control (.returned locals globals memory ffi [value]), nextClock) ∧
    panValuePcResultRelWithContextCode state.structs pcContext exceptionRel
      exceptionCode globalsLookup (.returned locals globals memory [value])
      (.returned targetState targetValues) := by
  exact evalPanValueFfiClockProgram_of_declarations_and_returned_call_context_code
    evaluatorContext initial clock (fun _ _ => none) evaluatorHandler fuel
    declarations entry arguments state shape locals globals memory ffi value nextClock
    pcContext exceptionRel exceptionCode globalsLookup targetState targetValues
    memoryAccess memoryHandler hdeclarations hcall hentry hshape hstate hvalues

/-! The timeout counterpart keeps the clock-exhaustion evaluator result and
    the context-aware source/target state relation visible together. -/
theorem timeout_call_context_code_fixture
    (initial : PanValueFfiProgramState Nat Unit)
    (clock : Nat) (fuel : Nat)
    (declarations : List (Decl Nat)) (entry : FunName)
    (arguments : List (Exp Nat))
    (state : PanValueProgramState Nat)
    (globals : VarName → Option (PanValue Nat))
    (memory : Nat → Option (PanValue Nat)) (ffi : FfiState Unit)
    (nextClock : Nat)
    (pcContext : CompileContext Nat)
    (exceptionRel : ExceptionId → PanValue Nat → Nat → Prop)
  ...
  exact evalPanValueFfiClockProgram_of_declarations_and_timeout_call_context_code
    evaluatorContext initial clock (fun _ _ => none) evaluatorHandler fuel
...
theorem related_declarations_returned_call_result_rel_fixture
    ...
    ∃ targetDeclaration,
      ...
      initial.ffi.ioEvents <+: ffi.ioEvents := by
  exact evalPanValueFfiClockProgram_of_related_declarations_and_returned_call_result_rel_ioEvents_prefix
    evaluatorContext initial clock (fun _ _ => none) evaluatorHandler fuel
    declarations entry arguments state shape locals globals memory ffi value nextClock
    pcContext exceptionRel exceptionCode globalsLookup targetState targetValues
    memoryAccess memoryHandler targetInitial hinitial hdeclarations hcall hentry
    hshape hstate hvalues hprefix

/-! Typed regression for the returned declaration-state context bridge.  The
    source declaration evaluator, transformed target state, returned payload,
    context state relation, and result relation all remain explicit for the
    `state_rel_imp_semantics_to_crep` returned branch. -/
theorem related_declarations_returned_call_context_code_adequacy_fixture
    (initial : PanValueFfiProgramState Nat Unit)
    (targetInitial : PanValueProgramState Nat)
    (clock : Nat) (fuel : Nat)
    (declarations : List (Decl Nat)) (entry : FunName)
    ...
    ∃ targetDeclaration,
      ...
      panValueCrepStateRelWithContext state.structs pcContext locals globals
        memory targetState ∧
      initial.ffi.ioEvents <+: ffi.ioEvents := by
  exact evalPanValueFfiClockProgram_of_related_declarations_and_returned_call_context_code_adequacy
    evaluatorContext initial clock (fun _ _ => none) evaluatorHandler fuel
    declarations entry arguments state shape locals globals memory ffi value nextClock
    pcContext exceptionRel exceptionCode globalsLookup targetState targetValues
    memoryAccess memoryHandler targetInitial hinitial hdeclarations hfunction hcall hentry
    hshape hstateContext hvalues hprefix

/-! Timeout uses the dedicated Pc timeout result (not a control-result
    constructor), so its declaration-state fixture keeps the explicit none
    locals, evaluator equation, post-state relation, and event prefix. -/
theorem related_declarations_timeout_call_result_rel_fixture
    (initial : PanValueFfiProgramState Nat Unit)
    (targetInitial : PanValueProgramState Nat)
    (clock : Nat) (fuel : Nat)
    (declarations : List (Decl Nat)) (entry : FunName)
  ...
  exact evalPanValueFfiClockProgram_of_related_declarations_and_timeout_call_result_rel_adequacy
    ...
    declarations entry arguments state globals memory ffi nextClock pcContext
    exceptionRel exceptionCode globalsLookup targetState memoryAccess memoryHandler
    targetInitial hinitial hdeclarations hcall hstate hprefix

/-! FinalFFI retains the terminal event equality in addition to the
    declaration-state, evaluator, ordinary-state, and event-prefix premises. -/
theorem related_declarations_finalFfi_call_result_rel_fixture
    (initial : PanValueFfiProgramState Nat Unit)
    (targetInitial : PanValueProgramState Nat)
    (clock : Nat) (fuel : Nat)
    (declarations : List (Decl Nat)) (entry : FunName)
    ...
    (locals globals : VarName → Option (PanValue Nat))
    (memory : Nat → Option (PanValue Nat)) (ffi : FfiState Unit)
    (event targetEvent : FfiFinalEvent) (nextClock : Nat)
    (pcContext : CompileContext Nat)
    (exceptionRel : ExceptionId → PanValue Nat → Nat → Prop)
    (exceptionCode : ExceptionId → Option Nat)
    (globalsLookup : CrepState Nat → PanValue Nat → Option (List Nat))
    ...
    ∃ targetDeclaration,
      ...
      initial.ffi.ioEvents <+: ffi.ioEvents := by
  exact evalPanValueFfiClockProgram_of_related_declarations_and_finalFfi_call_result_rel_ioEvents_prefix
    evaluatorContext initial clock (fun _ _ => none) evaluatorHandler fuel
    declarations entry arguments state locals globals memory ffi event nextClock
    pcContext exceptionRel exceptionCode globalsLookup targetState targetEvent
...
#check @Flapjack.evalPanValueFfiProgSteps_raise_ioEvents_prefix
#check @Flapjack.evalPanValueFfiProgSteps_return_ioEvents_prefix
...
#check @Flapjack.panSemEvaluate_ioEvents_prefix_of_leaf
