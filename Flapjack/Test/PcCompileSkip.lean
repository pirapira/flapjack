...
/-- The generic nested-record raise wrapper is applicable to a nested one-word
payload: it needs the exception lookup, freshness, payload exception relation,
flat-globals identity and the explicit values-carrying flat-global raised-data
evaluator evidence, with the source/global/memory state still visible. -/
example
    (hevidence : FlatGlobalRaisedEvidenceValues 1 0 0 skipNatExceptionCode)
    (hlookupException : ∀ (context : CompileContext Nat),
      ∃ exceptionCode, lookupInfo "E" context.exceptions = some exceptionCode)
    (hfresh : ∀ (context : CompileContext Nat) (state : CrepState Nat),
      state.locals (context.maxVar + 1) = none)
    (hexception : ∀ (context : CompileContext Nat)
      ...
      (exceptionCode : Nat),
      lookupInfo "E" context.exceptions = some exceptionCode →
      exceptionRel "E" (.rStruct [.rStruct [.word 9]]) exceptionCode)
    (hlookup : ∀ (targetState : CrepState Nat) (sourceValue : PanValue Nat),
      crepPcFlatGlobalsLookup 1 targetState sourceValue =
        skipNatGlobalsLookup targetState sourceValue) :
    PanValuePcCompileCorrect
      ...
      (crepPcCompactTargetEvaluator [] skipNatCrepPrimitive skipNatFfi
        skipNatSharedMem 0 0 4)
      skipNatCodeRel skipNatExcpRel skipNatExceptionCode skipNatGlobalsLookup
      (.raise "E" (.rStruct [.rStruct [.const 9]])) :=
  panValuePcCompileCorrect_compact_with_flat_global_evaluator_evidence_nested_one_word_raise
    "E" 9 skipNatCodeRel skipNatExcpRel skipNatExceptionCode
    skipNatGlobalsLookup [] [] skipNatPrimitive skipNatSourceHandler
    skipNatCrepPrimitive skipNatFfi skipNatSharedMem 0 0 1 4 4
    hlookupException hfresh hexception hlookup hevidence
