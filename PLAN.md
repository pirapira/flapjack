...
| Calls and FFI | Callee globals/memory and FFI state are threaded; call results/handlers are shape-checked; external calls read/write byte arrays | Structured, stepped, flat, and stateful-FFI evaluators now propagate callee globals/memory (and stateful FFI state where applicable). Generic structured and stepped evaluators now enforce declaration-driven return, exception, and handler shapes; generic call destinations validate local/global shapes and stand-alone calls discard returned values. Flat/stateful declaration-contract integration and remaining control checks remain ([#386](https://github.com/pirapira/flapjack/issues/386), [#388](https://github.com/pirapira/flapjack/issues/388), [#406](https://github.com/pirapira/flapjack/issues/406)) |
...
3. **Expression and statement agreement.** Model-aware structured and stepped
   ...
   contract environment into flat and stateful paths. Add the
   missing `store32`/
...
5. **Control-state fidelity.** Thread CakeML's clock and timeout rules through
   ...
   declaration environments for return, exception, and handler contracts;
   integrate those contracts into flat/stateful paths and complete the
   remaining control checks.
