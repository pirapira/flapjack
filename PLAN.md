...
| Calls and FFI | Callee globals/memory and FFI state are threaded; call results/handlers are shape-checked; external calls read/write byte arrays | Structured, stepped, flat, and stateful-FFI evaluators now propagate callee globals/memory (and stateful FFI state where applicable). Generic structured, stepped, and stateful-FFI evaluators now enforce declaration-driven return, exception, and handler shapes; generic call destinations validate local/global shapes and stand-alone calls discard returned values. Flat declaration-contract integration and remaining control checks remain ([#386](https://github.com/pirapira/flapjack/issues/386), [#388](https://github.com/pirapira/flapjack/issues/388), [#406](https://github.com/pirapira/flapjack/issues/406)) |
...
3. **Expression and statement agreement.** Model-aware structured and stepped
   ...
   contract environment into flat paths. Add the
   missing `store32`/
...
5. **Control-state fidelity.** Thread CakeML's clock and timeout rules through
   ...
   integrate those contracts into flat paths and complete the
