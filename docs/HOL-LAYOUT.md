...
| `compiler/backend/word_to_stackScript.sml` | `Flapjack/Compiler/Backend/WordToStack.lean` (`bits_to_word`, `word_list`); untagged executable bridge in `Flapjack/RiscV/CakeAllocatorBitsBridge.lean`; `state_rel`/`compile_semantics` remain open |
