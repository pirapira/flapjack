...
| `compiler/backend/stackLangScript.sml` | `Flapjack/Compiler/Backend/StackLang.lean` (`store_name`; generic `prog` syntax, `list_Seq`; shared-word `ProgW` carrier in `Flapjack/Compiler/Backend/StackProgW.lean`; exact width-indexed `prog` open under `flapjack-pxn.18.5.15.3.11`) |
...
| `compiler/encoders/asm/asmScript.sml` | `Flapjack/Compiler/Encoders/Asm.lean` (asm_config validity predicates: `reg_ok`, `fp_reg_ok`, `reg_imm_ok`, `offset_ok`, `arith_ok`, `fp_ok`, `cmp_ok`, `inst_ok`; exact width-indexed payload carriers `reg_imm`, `addr`, `inst`) |
