...
| `compiler/backend/wordLangScript.sml` | `Flapjack/Pancake/WordLang.lean` (`word_loc` width-indexed `WordLocW` (`[NeZero width]`) plus generic bridge; `raise_stub_location`, `store_consts_stub_location`; backend syntax) |
| `compiler/backend/backend_commonScript.sml` | `Flapjack/Compiler/Backend/BackendCommon.lean` (`word_add_carry`, `stack_num_stubs`, `word_num_stubs`) |
