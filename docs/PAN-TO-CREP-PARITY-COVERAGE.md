...
| `compile_prog` | [`compile_prog_probe.out`](../scripts/hol-probes/compile_prog_probe.out) | [`CompileProgParity.lean`](../Flapjack/Test/CompileProgParity.lean) | empty input; inline calls including first duplicate binding and nested expansion; valid Global return destination; handled call with missing destination and a two-word payload |
