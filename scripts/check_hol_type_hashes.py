...
def exported_types() -> list[dict[str, Any]]:
    # Use Lake's native `lean` command so the exporter sees the current
    # workspace build graph. A saved setup file can continue pointing at a
    # shared-cache OLean from an earlier source revision after `lake build`
